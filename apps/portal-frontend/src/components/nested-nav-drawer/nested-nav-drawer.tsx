...
import { getNavLinks } from '@deps/config/nav.config';
...
export default function NestedNavDrawer({ planCode, policyId }: NestedNavDrawerProps) {
    const { t } = useTranslation();
    const ref = useRef<HTMLElement>(null);

    const { isNavDrawerOpen, setIsOpenOverride, setOverrideAndStorage, shouldOverlay } = useStaticNestedNavDrawerContext();

    const links = useMemo(() => getNavLinks(planCode, policyId, t), [planCode, policyId, t]);
    ...
    const semanticNavWindowClasses = clsx('height-adjusted absolute z-20 bg-gray-900 transition-width duration-300', {

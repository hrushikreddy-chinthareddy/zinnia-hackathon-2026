import { Address, Tag } from '@zinnia/bloom/components';
import { AddressProps } from '@zinnia/bloom/components';
import dayjs from 'dayjs';
import { TFunction, useTranslation } from 'next-i18next';
import { ReactNode } from 'react';

import FieldData from '@deps/components/fields/field-data/field-data';
import { PeopleChangeType, getChangedParty, getPeopleChangeType } from '@deps/components/history-event-card/history-event-card.helper';
...
import { convertKebabedDateString, formatAccountNumber } from '@deps/helpers/string.helper';
import {
...
import { DEFAULT_ERROR_STRING, DEFAULT_EXTENDED_DAY_DATE_FORMAT, ZAHARA_API_DATE_FORMAT } from '@deps/types/constants';

export type NonFinancialTransactionSidesheetProps = {
    policy: Policy;
...
export default function SideSheetNonFinancialTransaction({ policy, transaction }: NonFinancialTransactionSidesheetProps) {
    const { t } = useTranslation();

    const effectiveDate = convertKebabedDateString(transaction.effectiveDate);

...
const ToggleLabel = ({ label }: Pick<ToggleProps, 'label'>) => {
    ...
    return <p className="font-primary text-sm font-bold leading-4.5 text-gray-900">{label}</p>;
};

const ToggleText = ({ text }: Pick<ToggleProps, 'text'>) => {
    if (!text) return null;
    return <p className="font-secondary text-md font-normal leading-5.5 text-gray-900">{text}</p>;
};

const ToggleInterior = ({ size, variant, value }: Pick<ToggleProps, 'size' | 'variant' | 'value'>) => {
    const baseClasses = `${
        variant === ToggleVariant.Inactive && !value ? 'bg-gray-100' : 'bg-white'
    } pointer-events-none inline-block transform rounded-full shadow ring-0`;
    if (size !== ToggleSize.Large) return <span className={`${baseClasses} h-4 w-4`} />;

    const iconClasses = 'text-white';
    if (value)
        return (
            <>
                <CheckmarkIcon width={20} height={20} className={`${iconClasses} mr-4`} />
    ...
    return (
        <>
            <span className={`${baseClasses} h-6 w-6`} />
            <CancelIcon width={20} height={20} className={`${iconClasses} ml-4`} />
        </>
    );
};
...
export default function Toggle({
    ...
    const baseClasses = `${focusClasses} ${
        variant === ToggleVariant.Inactive ? 'pointer-events-none' : 'pointer-events-auto'
    ...
    const selectedClasses = `${size === ToggleSize.Large ? 'pl-2.5' : 'pl-6'} ${
        variant === ToggleVariant.Inactive ? 'bg-primary-lightest' : 'bg-primary'
    } pr-1.5 `;
    const unselectedClasses = `${size === ToggleSize.Large ? 'pr-2.5' : 'pr-6'} ${
    ...
    return (
        <div className={clsx("flex flex-col gap-4", classes)}>
            ...
            <div className="flex items-center gap-2">
                <button
                    type="button"
                    className={`${baseClasses} ${value ? selectedClasses : unselectedClasses}`}
                    ...
                    role="button"
                >
                    <ToggleInterior size={size} variant={variant} value={value} />
                </button>

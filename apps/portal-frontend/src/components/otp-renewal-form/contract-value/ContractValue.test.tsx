...
describe('Contract Value Component ', () => {
    it('should renders the component with field label and input field', () => {
        ...
        const contractValueInput = screen.getByTestId('field-input-test-id').querySelector('input');
        ...
        expect(screen.getByText(`contractValue`)).toHaveClass('field-label text-gray-900 text-md mr-2');
    });

    it('should update input by typed numeric value', () => {
        render(<ContractValue />);
        const input = screen.getByTestId('contract-value-test-id') as HTMLInputElement;

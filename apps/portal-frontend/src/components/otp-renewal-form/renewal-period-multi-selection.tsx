...
export default function RenewalPeriodMultiSection({ options, isFormStateReadOnly, planCode }: RenewalPeriodMultiSectionProps) {
    ...
    const [fundAllocations, setFundAllocations] = useState<RenewalFund[]>([]);
    ...
    useEffect(() => {
        const getFundsList = async () => {
            try {
                ...
                const uniqueFunds = Array.from(new Set(fundsList.map(fund => fund.fundName))).map(item => {
                    return { fundName: item, value: null };
                });
            ...
            >
                {fundAllocations.map((fundAlloc, index) => (
                    <div key={index} className="my-2 grid grid-cols-2 gap-4">
                        ...
                        <Field
                            aria-labelledby={`fund-code-${fundAlloc.fundName}`}
                            onChange={e => {

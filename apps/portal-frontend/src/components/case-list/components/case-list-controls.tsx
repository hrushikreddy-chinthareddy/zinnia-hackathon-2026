...
import { DEFAULT_OFFSET_COUNT, DEFAULT_PAGE_LIMIT, caseListSearchPageSizeOptions } from '../helpers/const';
...
export const CaseListControls = ({ filters, setFilters, total }: CaseListControlsProps) => {
        ...
        if (filters) {
            const goToPage = (pageNumber: number) => {
                setFilters((fs: CaseSearchBody) => ({
                    ...fs,
                    offset: (pageNumber - 1) * (filters?.limit ?? DEFAULT_PAGE_LIMIT),
                }));
                window.scrollTo(0, 0);
            };
    ...
    return (
        <>
            {total ? (
                <div className="align-center mx-auto mt-4 grid grid-cols-4 lg:grid-cols-12 lg:pb-[120px]" data-testid="case-list-controls">

...
describe('MartialStatusAllowancesWithholdings', () => {
    ...
    it('should render marital status allowances component', () => {
        ...
        const singleCheckbox = screen.getByTestId(`marital-status-test-id-${MaritalStatusAllowances.Single}`);
        expect(singleCheckbox).toBeInTheDocument();
        expect(singleCheckbox).not.toBeChecked();

        const marriedCheckbox = screen.getByTestId(`marital-status-test-id-${MaritalStatusAllowances.Married}`);
        expect(marriedCheckbox).toBeInTheDocument();
        expect(marriedCheckbox).not.toBeChecked();

        const heaOfHousholdCheckbox = screen.getByTestId(`marital-status-test-id-${MaritalStatusAllowances.HeadOfHousehold}`);
        expect(heaOfHousholdCheckbox).toBeInTheDocument();
        expect(heaOfHousholdCheckbox).not.toBeChecked();

        const noOfAllowanceInput = screen.getByTestId('no-of-allowances-test-id');
        expect(noOfAllowanceInput).toBeInTheDocument();
        expect(noOfAllowanceInput).toHaveValue(maritalAllowances.exemption.text);
    });

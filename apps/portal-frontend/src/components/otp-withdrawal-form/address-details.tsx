...
import { AddressTypes, FormValidationErrors, Party } from '@deps/models/case/withdrawal/case';
...
const AddressDetails = ({ errors, formData, onDataChange }: AddressDetailsProps) => {
    ...
    useEffect(() => {
        ...
        addresses[0] = { ...address, addressLine1, addressLine2, city, state, zip, addressType: 'DEFAULT' as AddressTypes };
        ...
        <CardContainer classNames="w-full" containerClassNames="border-b-2 border-gray-100">
            ...
            <div className="my-4  flex max-w-lg">
                <Field
                    ...
                    value={addressLine1}
                    variant={errors.addressLine1 ? FieldVariant.Error : FieldVariant.Default}
                />
            </div>
            {address.addressLine2 && (
                <div className="my-4  flex max-w-lg">
                    <Field
                        label={t(`${BASE_TRANSLATION_KEY}mailingAddressLine2`) as string}
                        message={errors.addressLine2}
                        onChange={e => setAddressLine2(e.target.value)}
                        size={FieldSize.Small}
                        type={FieldType.BaseActive}
                        value={addressLine2}
                        variant={errors.addressLine2 ? FieldVariant.Error : FieldVariant.Default}
                    />
                </div>
            )}
            <div className="my-t grid max-w-lg grid-rows-3 gap-8 md:flex md:max-w-none md:flex-row">
                <Field
                    ...
                    type={FieldType.BaseActive}
                    value={city}
                    variant={errors.city ? FieldVariant.Error : FieldVariant.Default}
                />
                <Field
                    ...
                    type={FieldType.BaseActive}
                    value={state}
                    variant={errors.state ? FieldVariant.Error : FieldVariant.Default}
                />
                <Field
                    ...
                    variant={errors.zip ? FieldVariant.Error : FieldVariant.Default}
                />
            </div>
        </CardContainer>
    );

...
describe('ApiErrorCard', () => {
    ...
    it('navigates to correct route when "leaveTransaction" button is clicked', async () => {
        render(<ApiErrorCard leaveRoute="/policies/ABC/123/transactions/premiums" submit={{ action: () => {}, text: 'Submit' }} />);

        fireEvent.click(screen.getByText('leaveTransaction'));

        await waitFor(() => {
            expect(pushMock).toHaveBeenCalledWith('/policies/ABC/123/transactions/premiums');
        });
    });

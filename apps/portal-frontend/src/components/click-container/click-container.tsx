...
const ClickContainer: React.FC<ClickContainerProps> = props => {
    const containerClasses = clsx('rounded border-2 border-gray-100 bg-white !p-0', props.classes);

    return (
        <ClickWrapper {...props} classes={containerClasses} data-testid={props.testId}>
            <div className="p-4" data-testid="bank-details-container">{props.children}</div>
        </ClickWrapper>
    );
};

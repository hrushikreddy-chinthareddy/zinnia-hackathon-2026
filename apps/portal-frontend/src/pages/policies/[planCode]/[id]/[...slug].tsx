...
const PolicyDetailsPage: React.FC<PolicyPageProps> = () => {
    ...
    const [canEditEverly, setCanEditEverly] = useState(false);
        ...
        switch (slug[0]) {
            case 'people':
                ...
                subPageContent = slug[1] ? (
                    // TODO: remove the hardcoded 'everly' carrier ID once the backend can provide a proper carrier ID
                    <PersonSubPage editable={canEditEverly} partyId={slug[1]} />
                ) : (
                    <PeopleSubPage />
                );
            ...
            case 'policy':
                ...
                if (slug[1] === 'funds') {
                    subPageContent = <FundsSubPage policy={policy} />;
    ...
    return (
        <PolicyLayout policyDetails={policy}>
            <PolicyData.Provider value={{ policy, refreshPolicy }}>

...
const PolicyExtrasSubPage = () => {
    ...
    const chips = calculaterFilterProps({ riders: policy.riders, features: policy.policyFeatures, t });
            ...
            <CardContainer classNames="flex flex-col gap-4">
                <div>
                    ...
                    >
                        ...
                        {chips.map(chip => (
                            <RadioGroup.Item key={`extras-filter-${chip.value}`} value={chip.value} className="chip" disabled={chip.disabled}>
                ...
                <div>
                    <PolicyExtrasCards policy={policy} filterValues={selectedChip === ExtraFilters.All ? null : filterValues} />
                </div>
            </CardContainer>
        </div>
    );

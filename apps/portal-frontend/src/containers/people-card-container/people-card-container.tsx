...
const mapDataToPeopleCard = ({ chipEntered, index, party, peopleCard }: MapDataToPeopleProps) => {
    const { partyType, firstName, lastName, organizationCode, fullName, tags, beneficiaryPercentage, partyId } = party;
    const { selectedTagList, accessibilityText, accessibilityClickText, planCode, policyNumber, isBeneficiarySelected, router } = peopleCard;
    ...
    return (
        <CardPeople
            ...
            onClick={() => goTo(`/policies/${planCode}/${policyNumber}/people/${partyId}`, router)}
            shouldFocus={index === 0 && chipEntered}
        />
    );
};

const PeopleCardContainer = ({ filteredData, peopleCardData, classNames }: PeopleCardContainerProps) => {
    const { chipEntered } = useContext(ChipEnterContext);
    return (
        <div className={`grid h-fit w-full grid-cols-1 gap-2 md:grid-cols-2 ${classNames}`}>
            {filteredData.map((nameTag, index) => mapDataToPeopleCard({ chipEntered, index, party: nameTag, peopleCard: peopleCardData }))}
        </div>
    );
};

...
export interface FundsFirstGlanceViewModel {
    // Total Account Value of the Policy (loanned and unloanned if no Loan on Policy)
    // policy.policyValues.endingAccountValue
    accountValue?: number;
    ...
    // Sum total of value in all funds
    // funds && funds.reduce((acc, { totalFundValue = 0 }) => (acc += totalFundValue), 0);

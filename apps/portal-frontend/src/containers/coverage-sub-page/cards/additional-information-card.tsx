...
const AdditionalInformationCard = ({ currency, netAmountAtRisk, planCode }: AdditionalInformationCardProps) => {
    const { t } = useTranslation();

    const currencyFormat: Intl.NumberFormatOptions = { style: 'currency', currency };
    const netAmountRiskValue = !isNullEmptyOrUndefined(netAmountAtRisk)
        ? numberFormatify(netAmountAtRisk as number, currencyFormat)
        ...
        <CardContainer containerClassNames="rounded-b">
            ...
            <div className="mt-4 flex flex-col gap-8 sm:flex-row lg:ml-8">
                ...
                <div>
                    ...
                    <Content details={planCode ?? DEFAULT_ERROR_STRING} variant={ContentVariant.BodySm} />
                </div>
            </div>
        </CardContainer>
    );

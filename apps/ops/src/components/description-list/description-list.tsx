...
const Highlighter = dynamic(
    () => import('@deps/components/highlighter/highlighter')
);
...
const DescriptionList = ({
    ...
    const popoverClasses = clsx(
        'line-clamp-1 break-all font-secondary text-md',
        valueClassName
    );
    ...
    const policyParty = parties?.find(
        (party) => party.partyId === owner?.partyId
    );
    // TODO MG: this should not be hardcoded
    const policyPartyArr =
        partyRoles?.filter((pr) => pr.partyId === 'Party_PI_1') || [];
    ...
    if (label === 'Upcoming monthly premium') {
        ...
        const amountAndDate = `${numberFormatify(textSections[0])} - ${
            textSections[1]
        }`;

        const formattedAccountType =
            textSections[3].charAt(0).toUpperCase() +
            textSections[3].slice(1).toLowerCase();
                ...
                <>
                    ...
                    <span className={popoverClasses} aria-hidden="true">
                        <NavElement
                            ...
                            href={`/policies/${textSections[4]}/policy/premiums`}
                        >
                            {formattedAccountType +
                                t('policySummary.endingIn') +
                                textSections[2]}
                        </NavElement>
                    </span>
                </>
    ...
    return (
        <dl
            {...props}
            className={`box-border flex w-[197px] min-w-[100px] flex-col ${classNames} ${margin}`}
        >
                ...
                <div className="flex items-center gap-2">
                    {label !== 'Primary phone' &&
                        ...
                        label !== 'Mailing address' && (
                            <label
                                className={`${labelClassName} field-label`}

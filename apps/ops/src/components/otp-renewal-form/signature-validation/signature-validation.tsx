...
export default function SignatureValidations({ configs, isFormStateReadOnly }: SignatureValidationsProps) {
    ...
    const shouldShowTitleField = (ownerType: string) => {
        ...
        if (ownerType === 'Joint') {
            ownerParty = parties.find(party => party.SrcRoleOptionIdDesc.includes(ownerType) && party.SrcRoleType === 0);
        } else {
            ownerParty = parties.find(party => party.SrcRoleOptionIdDesc === ownerType && party.SrcRoleType === 0);
        }
                        ...
                        <div key={index} className="mb-4">
                            <SingleSignature
                                ...
                                errors={{
                                    ...
                                    [SignatureFields.SignatureDate]: formErrors[`${owner?.type}-${SignatureFields.SignatureDate}`],
                                }}

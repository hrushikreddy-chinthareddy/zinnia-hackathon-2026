...
import Field, { FieldSize, FieldVariant, FieldType } from '@deps/components/fields/field';
...
export function useSignatureFields(signature: Signature, errors: FormValidationErrors) {
    ...
    useEffect(() => {
        setCurrentSignature({
            ...
            signDate: (signDate && dayjs(signDate, DATE_PICKER_FORMAT).format(ZAHARA_API_DATE_FORMAT)) || null,
    ...
    const nameField = ({ label, isFormStateReadOnly }: IFieldConfig) => (
        <Field
            ...
            type={FieldType.BaseActive}
            value={name}
            variant={selectVarientByConfig({ value: name, isFormStateReadOnly, error: errors[SignatureFields.Name]})}
        />
    );
    ...
    const signDateField = ({ label, isFormStateReadOnly }: IFieldConfig) => (
        <FieldDateSelect
            ...
            type={FieldType.BaseActive}
            value={signDate}
            variant={selectVarientByConfig({ value: signDate, isFormStateReadOnly, error: errors[SignatureFields.SignaturePresent]})}
            disabled={isFormStateReadOnly}
        />
    );
    ...
    const renderField = (field: SignatureFieldConfig, isFormStateReadOnly: boolean): JSX.Element | null => {
        switch (field.fieldName) {
            case SignatureFields.Name:
                return <div key={field.fieldName}>{nameField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
            case SignatureFields.SignatureType:
                return <div key={field.fieldName}>{signTypeField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
            case SignatureFields.SignaturePresent:
                return <div key={field.fieldName}>{signPresentField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
            case SignatureFields.SignatureDate:
                return <div key={field.fieldName}>{signDateField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
            case SignatureFields.SignatureTitle:
                return <div key={field.fieldName}>{signTitleField(isFormStateReadOnly)}</div>;
...
export function SingleSignature({ fields, showTitle = false, errors, signature, isFormStateReadOnly, onDataChange }: SingleSignatureProps) {
    const { renderField, currentSignature } = useSignatureFields(signature, errors);
    ...
    return (
        <>
            <div className="my-4 grid  grid-cols-4 gap-4">{fields?.map((field) => renderField(field, isFormStateReadOnly))}</div>
            {showTitle && <div className="my-4 w-1/3">{renderField({ fieldName: SignatureFields.SignatureTitle }, isFormStateReadOnly)}</div>}
        </>
    );
}

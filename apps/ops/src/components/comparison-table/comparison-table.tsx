...
const leftCellClasses = 'align-top border-t-2 border-l-2 border-gray-100 py-2 px-4 sm:px-6 text-left sm:w-[calc(500px/3)] md:w-fit ';
    ...
    return (
        <tr>
            ...
            <th scope="col" className={`rounded-t ${centerCellClasses}`}>
                <Content variant={ContentVariant.BodySmBold} details={headers.new as string} />
            </th>
            <th scope="col" className="px-6 py-2 text-left">
                <Content variant={ContentVariant.BodySmBold} details={headers.current as string} />
            </th>
        </tr>
    );
};
...
const BankDetailsTableRow: React.FC<TableRowsProps> = ({ rowData }) => {
    ...
    const isDifferent =
        newBankDetails && currentBankDetails ? isBankDataDifferent(newBankDetails as BankData[], currentBankDetails as BankData[]) : false;

...
import { FormSignature, FormValidationErrors, SignatureWithdrawal } from '@deps/models/case/withdrawal/case';
...
type SignatureValidationProps = {
    ...
    onSignatureSet: (signatureType: SignatureValidationTypeWithdrawal, val: SignatureWithdrawal) => void;
    ownerName?: string;
...
export default function SignatureValidationContainer({
    ...
    const getErrors = (signatureType: SignatureValidationTypeWithdrawal): FormValidationErrors => {
        return {
            [SignatureFieldNames.IsSignatureValid]: errorData[`${signatureType}${SignatureFieldNames.IsSignatureValid}`],
            [SignatureFieldNames.SignatureComment]: errorData[`${signatureType}${SignatureFieldNames.SignatureComment}`],
            [SignatureFieldNames.SignatureDate]: errorData[`${signatureType}${SignatureFieldNames.SignatureDate}`],
            [SignatureFieldNames.SignaturePresent]: errorData[`${signatureType}${SignatureFieldNames.SignaturePresent}`],
            [SignatureFieldNames.SignatureTitle]: errorData[`${signatureType}${SignatureFieldNames.SignatureTitle}`],
            [SignatureFieldNames.SignatureType]: errorData[`${signatureType}${SignatureFieldNames.SignatureType}`],
        };
    };
    ...
    return (
        <>
            {config
                .filter(item => (item?.shouldDisplay ? item?.shouldDisplay() : true))
                .map(({ signatureType, fields, bonusField }) => {
                    // TODO: Init default signature in Signature provider context.
                    const signProcessing =
                        signatureData.signatures.find(val => val.signType?.text === signatureType) || getDefaultSignature(signatureType);
                    let signCl = signProcessing;
                    if (signProcessing.signType.text === SignatureValidationTypeWithdrawal.Owner && ownerName) {
                        signCl = { ...signProcessing, signName: ownerName };
                    }
                    if (signProcessing.signType.text === SignatureValidationTypeWithdrawal.JointOwner && jointOwnerName) {
                        signCl = { ...signProcessing, signName: jointOwnerName };
                    }
                    if (signProcessing.signType.text === SignatureValidationTypeWithdrawal.Annuitant && annuitantName) {

...
import caseChartHelpers, {
    ChartConfigSeriesDataSimple,
} from '@deps/helpers/dashboard/case-chart-helpers';
...
const ActiveAgingBars = forwardRef<HighchartsReactRefObject, Props>(
    ({ agingRangesByProcess, classNames, onRenderChart }, ref) => {
        ...
        const getSeriesData = (
            ...
            if (
                !agingRangesByProcess ||
                !agingRangesByProcess.data ||
                agingRangesByProcess.data.length === 0
            ) {
                return seriesData;
            }
        ...
        const getChartConfig = useCallback(
            (seriesData: ChartConfigSeriesDataSimple[]) => {
                const seriesDataWithLabelFormatted = seriesData.map(
                    (series: ChartConfigSeriesDataSimple) => {
                        return {
                            ...series,
                        ...
                        formatter: function (
                            ...
                            returnString += `${
                                this.series.userOptions.name
                                    ? this.series.userOptions.name
                                    : 'Unknown'
                            }: ${wholeNumberFormatify(this.y)}<br/>`;
        ...
        useEffect(() => {
            ...
            getChartConfig,
            agingRangesByProcess,
            agingRangesByProcess?.data?.length,
        ]);

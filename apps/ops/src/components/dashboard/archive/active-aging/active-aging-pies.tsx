...
const ActiveAgingPies = ({
    dashboardStatsResponse,
    classNames,
    width,
    distinctPieChartCategoryStatGroupingLabels,
}: Props) => {
    return (
        <div className="relative flex justify-end">
            <Typography
                className="-rotate-90 absolute text-center -left-[20px] top-[50px]"
                variant={TypographyVariant.BodyBold}
            >
                Product Name
            </Typography>
            <div
                className={`${classNames} border-b-1 border-[#ddd]`}
                style={{ width: `${width}px` }}
            >
                <div className="grid grid-cols-6 gap-2 w-full justify-items-center">
                    {dashboardStatsResponse?.data?.map(
                        (currentStatGrouping) => {
                            const additionalStats: DashboardStatsElementResponse[] =
                                [];

                            distinctPieChartCategoryStatGroupingLabels.forEach(
                                (currentLabel) => {
                                    if (
                                        !currentStatGrouping.values?.find(
                                            (stat) =>
                            ...
                            if (currentStatGrouping.values) {
                                currentStatGrouping.values = [
                                    ...currentStatGrouping.values,
                                    ...additionalStats,
                                ];
                                currentStatGrouping.values?.sort((a, b) =>
                                    a.name
                                        .toLowerCase()

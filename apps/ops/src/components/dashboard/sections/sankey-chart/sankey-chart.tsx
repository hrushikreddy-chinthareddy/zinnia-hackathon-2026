...
const SankeyChart = ({
    ...
    const [level1ObjectGrouping, setLevel1ObjectGrouping] = useState<
        CaseCountOutputLevel1[]
    >([]);
    const [level2ObjectGrouping, setLevel2ObjectGrouping] = useState<
        CaseCountOutputLevel1[]
    >([]);
    const [level3ObjectGrouping, setLevel3ObjectGrouping] = useState<
        CaseCountOutputLevel1[]
    >([]);
    const [l1SelectedIndex, setL1SelectedIndex] = useState<number>(-100);

    const [l1SelectValue, setL1SelectValue] = useState(
        DEFAULT_GROUPBY_FILTER_OPTIONS.L1SelectValue
    );
    const [l2SelectValue, setL2SelectValue] = useState(
        DEFAULT_GROUPBY_FILTER_OPTIONS.L2SelectValue
    );
    const [l3SelectValue, setL3SelectValue] = useState(
        DEFAULT_GROUPBY_FILTER_OPTIONS.L3SelectValue
    );
    ...
    const {
        ...
        queryFn: () => {
            ...
            return getStatsFromSelectionQuery(
                baseDashboardQueryFilter,
                l1SelectValue,
                l2SelectValue,
                l3SelectValue
    ...
    const getGroupingsFromL1 = (l1ObjectGrouping: CaseCountOutputLevel1[]) => {
        ...
        l1ObjectGrouping.forEach((l1Grouping) => {
            if (
                ...
                l1Grouping.values &&
                l1Grouping.values &&
                l1Grouping.values.length > 0
            ) {
                l1Grouping.values.forEach((l2ChildGrouping) => {
                    const clonedL2ChildGrouping: CaseCountOutputLevel1 =
                        JSON.parse(JSON.stringify(l2ChildGrouping));
                    tempL2Grouping.push(clonedL2ChildGrouping);

                    clonedL2ChildGrouping?.values?.forEach(
                        (l3ChildGrouping) => {
                            const clonedL3ChildGrouping = JSON.parse(
                                JSON.stringify(l3ChildGrouping)
                            );
        ...
        tempL2Grouping.forEach((grouping) => {
            if (grouping) {
                ...
                if (
                    !l2Grouping.find(
                        (l2Group) => l2Group.name === grouping.name
                    )
                ) {
        ...
        tempL3Grouping.forEach((grouping) => {
            if (grouping) {
                if (
                    !l3Grouping.find(
                        (l3Group) => l3Group.name === grouping.name
                    )
                ) {
        ...
        Object.keys(l2CountByKeyMap).forEach((key) => {
            const currentl2Grouping = l2Grouping.find(
                (l2Group) => l2Group.name === key
            );
        ...
        Object.keys(l3CountByKeyMap).forEach((key) => {
            const currentL3Grouping = l3Grouping.find(
                (l2Group) => l2Group.name === key
            );
    ...
    const calculateLevel1PathCoordinates = (
        level1Index: number,
        level2Index: number
    ) => {
        const level1BlockVPosition = calculateVerticalBlockPosition(
            level1Index,
            level1ObjectGrouping.length
        );
        const level2BlockVPosition = calculateVerticalBlockPosition(
            level2Index,
    ...
    const calculateLevel2PathCoordinates = (
        level2Index: number,
        level3Index: number,
        pathIndex: number
    ) => {
        ...
        const level3BlockVPosition = calculateVerticalBlockPosition(
            level3Index,
        ...
        const yStart =
            level2BlockVPosition +
            (blockheights - blockSpacing) / 2 +
            (pathIndex ?? 1) * pathSpacing;
    ...
    const renderL1Path = (
        l1StatGrouping: CaseCountOutputLevel1,
        l2StatGrouping: CaseCountOutputLevel2,
        level1Index: number
    ) => {
        const level2Index = level2ObjectGrouping.findIndex(
            (grouping) => grouping.name === l2StatGrouping.name
        );
        if (level2Index < 0) {
            return null;
        }
        ...
        let strokeWidth =
            (l2StatGrouping.count / l1StatGrouping.count) *
            (blockheights - blockSpacing);
        ...
        return (
            <path
                ...
                stroke={
                    isL1Selected() &&
                    !isMatchForL1SelectedStatGrouping(level1Index)
                        ? '#eee'
                        : getLevel1PathColor(level1Index)
                }
    ...
    const renderL2HoverPath = (
        ...
        // the path
        const level3Index = level3ObjectGrouping.findIndex(
            (grouping) => grouping.name === l3StatGrouping.name
        );
        if (
            level3Index < 0 ||
            !l3StatGrouping ||
            !l2StatGrouping ||
            !l2StatGrouping.values ||
            !l2StatGrouping.values
        ) {
            return null;
        }
        ...
        const l2MatchedItem = currentHoveredL1Item?.values?.find(
            (l2ChildStatGrouping) =>
                l2ChildStatGrouping.name === l2StatGrouping.name
        );
        const l3MatchingItem = l2MatchedItem?.values?.find(
            (l3ChildStatGrouping) =>
                l3ChildStatGrouping.name === l3StatGrouping.name
        );
        ...
        let strokeWidth = Math.floor(
            (l3MatchingItem.count / l2StatGrouping.count) *
                (blockheights - blockSpacing)
        );
        ...
        return (
            <path
                ...
                d={calculateLevel2PathCoordinates(
                    level2Index,
                    level3Index,
                    pathIndex
                )}
    ...
    const renderL2Path = (
        ...
        let strokeWidth =
            (l2StatGrouping.count / l3StatGrouping.count) *
            (blockheights - blockSpacing);
        strokeWidth = Math.min(strokeWidth, maxPathStrokeWidth); // don't exceed the max
        strokeWidth = Math.max(strokeWidth, minPathStrokeWidth); // don't be less than the min
    ...
    const renderL1Group = (
        l1StatGrouping: CaseCountOutputLevel1 | null,
        index: number,
        ...
        return (
            <g
                key={`level-1-${index}`}
                data-name={`level-1-${index}`}
                transform="translate(0.5,0.5)"
            >
                <a
                    href="#!"
                    title={l1StatGrouping.name}
                    onClick={(event) => handleL1Click(event, index)}
                >
                    <rect
                        x={svgleftAndRightPadding}
                        y={calculateVerticalBlockPosition(
                            index,
                            level1ObjectGrouping.length
                        )}
                        width={blockWidths}
                        ...
                        style={{
                            fill:
                                isL1Selected() &&
                                !isMatchForL1SelectedStatGrouping(index)
                                    ? '#eee'
                                    : getLevel1PathColor(index),
                        }}
                    />
                    <SankeyCellText
                        width={blockWidths}
                ...
                >
                    {l1StatGrouping &&
                        ...
                        l1StatGrouping.values.map((childStatGrouping) =>
                            renderL1Path(
                                l1StatGrouping,
                                childStatGrouping,
                                index
    ...
    const renderL2Group = (
        ...
        const l2MatchedItem = l1MatchedItem?.values?.find(
            (l2ChildStatGrouping) =>
                l2ChildStatGrouping.name === l2StatGrouping.name
        );
        ...
        return (
            <g
                ...
                style={{
                    visibility:
                        !selectedItem || isSelected ? 'visible' : 'hidden',
                }}
            >
                ...
                <g>
                    ...
                    <SankeyCellText
                        ...
                        height={blockheights}
                        transform={`translate(${
                            parentSize.width / 2 - blockWidths / 2
                        } ${calculateVerticalBlockPosition(
                            index,
                            level2ObjectGrouping.length
                        )})`}
                ...
                {level3ObjectGrouping &&
                    ...
                    level3ObjectGrouping.map((l3Item, innerIndex) =>
                        renderL2Path(l2StatGrouping, l3Item, index, innerIndex)
                    )}
                {l2MatchedItem &&
                    selectedItem &&
                    l2MatchedItem?.values?.map((l3Item, innerIndex) =>
                        renderL2HoverPath(
                            l2StatGrouping,
                            l3Item,
                            index,
        ...
        return (
            ...
            >
                ...
                <SankeyCellText
                    ...
                    height={blockheights}
                    transform={`translate(${
                        parentSize.width - blockWidths - svgleftAndRightPadding
                    } ${calculateVerticalBlockPosition(
                        index,
                        level3ObjectGrouping.length
                    )})`}
    ...
    const getL2ObjectColor = (item: CaseCountOutputLevel2, index: number) => {
        const matchingStatGrouping = getSelectedL1StatGrouping()?.values?.find(
            (statGrouping) => statGrouping.name === item.name
        );
    ...
    const isMatchForL1SelectedStatGrouping = (index: number) =>
        l1SelectedIndex === index;

    useEffect(() => {
        const handleWindowResize = () => {
            if (svgParentRef.current) {
                const { width, height } =
                    svgParentRef.current.getBoundingClientRect();
                setParentSize({
    ...
    useEffect(() => {
        if (
            !caseGroupingState ||
            !caseGroupingState.data ||
            caseGroupingState.data.length === 0
        ) {
        ...
        const totalCaseCount = l1Grouping.reduce(
    ...
    const resetFilters = () => {
        [setL1SelectValue, setL2SelectValue, setL3SelectValue].forEach(
            (setterFn, index) =>
    ...
    return (
        <BlurOverlayLoader loading={caseGroupingDataLoading || isFetching}>
            <div className="mt-8">
                <ChartHeader
                    title={t('caseStatCharHeader', {
                        count: formattedCasesNumber,
                    })}
                    subtitle={undefined}
                />
                        ...
                        >
                            <g data-name="level 1">
                                ...
                                {level1ObjectGrouping &&
                                    ...
                                    level1ObjectGrouping.map((item, index) =>
                                        renderL1Group(item, index)
                                    )}
                            ...
                            <g data-name="level 2">
                                {level2ObjectGrouping &&
                                    ...
                                    level2ObjectGrouping.map((l2Item, index) =>
                                        renderL2Group(l2Item, index)
                                    )}
                                {level2ObjectGrouping &&
                                    level2ObjectGrouping.length > 0 &&
                                    level2ObjectGrouping.map((l2Item, index) =>
                                        renderL2Group(l2Item, index, true)
                                    )}
                            </g>
                            <g data-name="level 3">
                                <g
                                    data-name="End Stages"
                                    transform="translate(0.5,0.5)"
                                    style={{ visibility: 'visible' }}
                                >
                                    {level3ObjectGrouping &&
                                        level3ObjectGrouping.length > 0 &&
                                        level3ObjectGrouping.map(
                                            (item, index) =>
                                                renderL3Group(item, index)
                                        )}
                                </g>
                            </g>
                        </svg>

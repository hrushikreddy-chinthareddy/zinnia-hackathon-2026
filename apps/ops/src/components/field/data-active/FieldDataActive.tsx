...
import { Label, AssistiveText, AssistiveTextVariant } from '@zinnia/bloom/components';
import clsx from 'clsx';
import { cloneElement, forwardRef } from 'react';
import { v4 as uuidv4 } from 'uuid';

import fieldStyles from '../Field.module.css';
import { FieldDataActiveProps, FieldDataActiveTestIds, FieldStatus } from '../types';

export const FieldDataActive = forwardRef<HTMLInputElement, FieldDataActiveProps>(
    ({ errorMessage = '', label, fieldStatus = FieldStatus.DEFAULT, fieldSize, icon, ...props }, ref) => {
            ...
            <>
                ...
                <div className={clsx(fieldStyles.inputContainer, fieldStyles[status], 'typography-content-body-sm')}>
                    <input
                        ...
                        type="text"
                        className={clsx(fieldStyles.input, fieldSize && fieldStyles[fieldSize], fieldStyles[status])}
                        {...props}
                    />
                    {icon && (
                        <div data-testid={FieldDataActiveTestIds.ICON} className={fieldStyles.rightIconContainer}>
                            {icon}
                        </div>
                    )}
                </div>

...
export function ObjectFieldTemplate(props: ObjectFieldTemplateProps) {
    const { expandable, help, defaultItemLabel, width } = getUiOptions(props.uiSchema);
    const helpText = help;
    ...
    const content = (
        <div className={`my-0 px-0 ${classes} `}>
            {props.properties
                ...
                .map(element => {
                    return (
                        <div key={element.name} className={width ? `${style[width as string]}` : 'property-wrapper flex flex-col'}>
                            {element.content}
                        </div>
                    );
                })}
        ...
        <>
            {expandable ? (
                <AccordionRoot type="single" value={props.title}>
                    <AccordionItem key={props.title} value={props.title} className="rounded-lg border-2 border-gray-100">
                        <AccordionHeader>
                            <AccordionTrigger
                                ...
                                <div className="flex gap-2 items-center">
                                    {openSection === true ? (
                                        <div onClick={() => setOpenSection(false)}>
                                            <Icon type={IconType.CHEVRON} height={20} width={20} />
                                        </div>
                                    ) : (
                                        <div onClick={() => setOpenSection(true)}>
                                            <Icon type={IconType.CHEVRON_RIGHT} height={20} width={20} />
                                        </div>
                                    )}
                                    <div className={style.text}>
                                        {(replacePlaceholders(props.title, props?.formData, true) || defaultItemLabel) ?? ''}
                                    </div>
                                    {props.required && (
                                        <span className={fieldStyles.requiredAsterisk}>
                                            {'\u00A0'}
                                            {'\u002A'}{' '}
                                        </span>
                                    )}
                        ...
                        <AccordionContent>
                            <div className="flex flex-col w-full justify-between border-b-2 border-gray-100 bg-gray-50 px-4 last:rounded-b-lg last:border-b-0">
                                {openSection && <div className="py-2">{content}</div>}
                            </div>
                        </AccordionContent>
                    </AccordionItem>
                </AccordionRoot>

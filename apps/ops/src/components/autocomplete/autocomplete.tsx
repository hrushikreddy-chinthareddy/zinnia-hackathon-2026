...
const Autocomplete = ({
    ...
    const selectedLabel = options?.length && (options as SimpleOption[]).find(option => option.value === value)?.label;
    ...
    return (
        <div className={clsx('flex w-full flex-col', className)}>
            ...
            <RadixSelect.Root {...rootProps}>
                <ComboboxProvider open={isOpen} setOpen={setIsOpen} resetValueOnHide includesBaseElement={false} setValue={setSearchValue}>
                    <RadixSelect.Trigger aria-label="options" className={`${variantClass} ${getTriggerClasses(isOpen)}`}>
                        ...
                        <div aria-hidden="true">
                            <FieldIcon
                                ...
                                className={clsx('simple-transition', 'mr-4', { flip180: isOpen })}
                            />
                        </div>
                    </RadixSelect.Trigger>
                    <AutocompleteOptions options={options} value={value || ''} matches={matches} contentClasses={contentClasses} />
                </ComboboxProvider>
            </RadixSelect.Root>

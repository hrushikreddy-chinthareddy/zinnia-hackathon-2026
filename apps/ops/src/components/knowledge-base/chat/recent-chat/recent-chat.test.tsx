...
describe('Recent Chat', () => {
  ...
  it('renders client name and recent chat heading', async () => {
    ...
    expect(getByText((content) => content.includes('Security Benefit'))).toBeInTheDocument();
  });
  ...
  it('fetches selected client chat sessions on render', async () => {
    ...
    await waitFor(() => {
      ...
      expect(getByText(mockChatSessions[0].sessionTitle)).toBeInTheDocument();
    });
  });
  ...
  it('logs errors when it fails to fetch selected client chat sessions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error fetching chat history', { error: testError })
    });
  });
  ...
  it('performs search on typing 3+ characters', async () => {
    ...
    const { getByPlaceholderText, getByText } = render(<RecentChat opsUserData={mockOpsUser} />);
  ...
  it('does not perform search if there are less than 3 characters', async () => {
    const { getByPlaceholderText } = render(<RecentChat opsUserData={mockOpsUser} />);
    const searchInput = getByPlaceholderText('sidenav.search');
  ...
  it('logs errors when it fails to fetch selected client chat sessions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error searching chat history', { error: testError })
    });
  });
  ...
  it('calls navigates on clicking an old chat session when on documents or admin page', async () => {
    ...
    await waitFor(() => {
      expect(mockPush).toHaveBeenCalledWith(`/zinnia-ai-assistant/chat?sessionId=1`);
    });
  });
  ...
  it('fetches more chat sessions on scroll when not in search mode', async () => {
    ...
    Object.defineProperty(scrollContainer!, 'clientHeight', { value: 10, writable: true });
  ...
  it('fetches more chat sessions on scroll when in search mode', async () => {
    ...
    const { getByPlaceholderText, container } = render(<RecentChat opsUserData={mockOpsUser} />);
    const searchInput = getByPlaceholderText('sidenav.search');
    fireEvent.change(searchInput, { target: { value: 'Chat' } });

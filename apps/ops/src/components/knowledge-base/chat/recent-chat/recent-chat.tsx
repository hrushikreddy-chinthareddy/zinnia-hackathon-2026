import { ChatSearchResponse, ChatSessionResponse, MeResponse } from "@xd/api-types/dist/generated-types/knowledgebase";
...
import { listChatSessionsByClientId, searchChatHistory } from "@deps/queries/api/knowledge-base";
...
const RecentChat = ({ opsUserData }: RecentChatsProps) => {
  ...
  const { selectedClientId, viewChatHistory, chatHistoryReloadTrigger, sessionId } = useKnowledgeBaseContext();
  ...
  const fetchChatHistory = useCallback(
    async (clientId: string, pageNum: number) => {
      ...
      try {
        ...
        if (res?.content) {
          setChatHistory((prev) =>
            pageNum === START_PAGE_NUM ? res.content : [...prev, ...res.content]
          );
    ...
    <div className="text-gray-500 border-t border-gray-100 mt-4">
      <div className="flex flex-col gap-2 my-4">
        ...
        <Typography variant={TypographyVariant.BodySm}>
          {clientName.replace(/\.aspx$/i, "").replace(/-/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2")}
        </Typography>
      </div>
      ...
      >
        {chatsToRender.length > 0 ? (
          chatsToRender?.map((chat) => {
            const { sessionId: itemSessionId, sessionTitle, lastActivity } = chat;
            return (
              <div tabIndex={0}
                key={itemSessionId}
                className={`flex gap-2 items-center py-2 cursor-pointer ${styles.itemhover} ${itemSessionId === sessionId ? styles.activeItem : ''}`}
                onClick={() => handleViewChatHistory(itemSessionId || '')}
                onKeyDown={(e) => {
              ...
              >
                ...
                <div className="flex flex-col">
                  <Typography variant={TypographyVariant.BodySmBold}>
                    {
                      sessionTitle && sessionTitle?.length <= SESSION_TITLE_MAX_LEN
                        ? sessionTitle
                        : `${sessionTitle?.slice(0, SESSION_TITLE_MAX_LEN)}...`
                    }
                  </Typography>
                  <Typography
          ...
          <Typography variant={TypographyVariant.BodySm} className="px-2">
            {isSearchActive

...
import { BOT_ERROR_MESSAGE_ID, ChatbotMessage, KeyboardEvents, MessageRole, UserMessage } from '@deps/types/knowledge-base';
...
const ChatInput = ({ opsUserData }: ChatInputProps) => {
  ...
  const { sessionId, setSessionId, selectedClientId, setCurrentMessages, setChatHistoryReloadTrigger, currentMessages } =
      ...
      <div className="w-full">
        <div className={`${styles.textboxContainer}`}>
          ...
          <div className='flex gap-2'>
            {
              loading ? <button aria-label='stop-button' type="button" onClick={() => handleStopResponse()}>{t('chat.stop')}</button>
                : <button aria-label='send-button' type="button" onClick={() => handleMessageSend(message)}>
                  {t('chat.send')}
                </button>
            }
            {
              currentMessages?.find((message: any) => message.id === BOT_ERROR_MESSAGE_ID) &&
              <button aria-label='retry-button' type="button" onClick={() => retryLastMessage()}>{t('chat.retry')}</button>
            }
          </div>
        </div>
        <Typography

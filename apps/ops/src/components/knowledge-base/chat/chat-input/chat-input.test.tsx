...
import { createNewChatSession, getChatbotResponse } from "@deps/queries/api/knowledge-base";
...
describe('ChatInput', () => {
  ...
  it('renders the chat input field and button', async () => {
    const { getByRole } = render(<ChatInput opsUserData={mockOpsUserData} />);
  ...
  it('calls getChatbotResponse when there is a sessionId', async () => {
    ...
    await waitFor(() => {
      expect(getChatbotResponse).toHaveBeenCalledWith('existing-session', mockMessage, 'client-123', expect.any(AbortSignal));
      ...
      const updatedMessages = lastCallArg([{ id: 'new_user', role: MessageRole.User, content: mockMessage }]);
  ...
  it('logs error and shows fallback bot message when getChatbotResponse fails', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error getting response from chatbot', {
        sessionId: '1234567890'
      });
      expect(mockSetCurrentMessages).toHaveBeenCalledWith(expect.any(Function));

      const updateFn = mockSetCurrentMessages.mock.calls[1][0];
      const updatedMessages = updateFn([
        { id: 'new_user', role: MessageRole.User, content: mockMessage }
      ]);
  ...
  it('logs error when fails to send a message', async () => {
    ...
    await waitFor(() => {
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error sending message::', {
        error: testError
      });
    });
  });
  ...
  it('stops the api call when stop button is pressed and retries the api call with same question when retry button is pressed', async () => {
    ...
    await waitFor(() => {
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Chatbot response request cancelled');
    });
  });
  ...
  it('retries the API call with the same question when retry button is pressed', async () => {
    ...
    const { getByRole, rerender } = render(<ChatInput opsUserData={mockOpsUserData} />);
    const input = getByRole('textbox');
    ...
    mockUseKnowledgeBaseContext.mockReturnValue({
      ...
      currentMessages: [
        { id: 'new_user', role: MessageRole.User, content: mockMessage },
        { id: BOT_ERROR_MESSAGE_ID, role: MessageRole.Bot, content: 'chat.errorMsg' }
      ],
      chatHistoryReloadTrigger: 0,
      setChatHistoryReloadTrigger: jest.fn(),
      setSelectedClient: jest.fn(),

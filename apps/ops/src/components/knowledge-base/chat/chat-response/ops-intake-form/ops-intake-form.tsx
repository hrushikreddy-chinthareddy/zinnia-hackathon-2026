...
import { DislikeReasonsPayload, OpsIntakeFormPayload } from "@deps/types/knowledge-base";
...
const TextField = ({
  ...
  const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'zinniaAiAssistant.chat.feedback' });
...
const OpsIntakeForm = ({ metadata, setOpenOpsIntakeForm, dislikeReason, setMetadata, onDislikeReasonChange }: OpsIntakeFormProps) => {
  const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'zinniaAiAssistant.chat.feedback', });
  const [processName, setProcessName] = useState(metadata.processName || '');
  const [blockOfBusiness, setBlockOfBusiness] = useState(metadata.blockOfBusiness || '');
  const [processDescription, setProcessDescription] = useState(metadata.processDescription || '');
  const [outcomeExpected, setOutcomeExpected] = useState(metadata.outcomeExpected || '');
  const [smeEmail, setSmeEmail] = useState(metadata.smeEmail || '');
  const [priority, setPriority] = useState<Priority>(metadata.priority as Priority || Priority.Low);
  const [requestFrequency, setRequestFrequency] = useState<RequestFrequency>(metadata.requestFrequency as RequestFrequency || RequestFrequency.Daily);
  const [benefitMetrics, setBenefitMetrics] = useState(metadata.benefitMetrics || '');
  ...
  const validateOpsIntakeForm = () => {
    return Boolean(
      ...
      smeEmail.trim() && !validateEmail(smeEmail) &&
      priority &&
  ...
  const requestFrequencyRadioOptions = Object.values(RequestFrequency).map(p => ({
    label: p,
    ariaLabel: p,
    value: p,
  }));
  ...
  return (
    <div className="h-full overflow-y-auto my-4 mx-4">
      ...
      <div className="flex flex-col gap-6">
        <TextField label={'opsIntakeForm.processName'} value={processName} onChange={setProcessName} />

        <div className="flex flex-col gap-2">
          <TextField label={'opsIntakeForm.blockOfBusiness'} value={blockOfBusiness} onChange={setBlockOfBusiness} />
          <AssistiveText
            text={t('opsIntakeForm.blockOfBusinessAssistiveText') || ''}
        ...
        <div className="flex flex-col gap-2">
          <FieldLabel label={t('opsIntakeForm.processDescription') || ''} required />
          <textarea
        ...
        <TextField label={'opsIntakeForm.outcomeExpected'} value={outcomeExpected} onChange={setOutcomeExpected} />
        <TextField label={'opsIntakeForm.smeEmail'} value={smeEmail} onChange={setSmeEmail}
          message={(smeEmail && validateEmail(smeEmail) && t(validateEmail(smeEmail) as string)) ?? ''} />
        ...
        <div className="flex flex-col gap-2">
          <Radio
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRequestFrequency(e.target.value as RequestFrequency)}
            required />
          <AssistiveText
            text={t('opsIntakeForm.requestFrequencyAssistiveText') || ''}
            variant={AssistiveTextVariant.Brand}
            iconOverride={<Icon type={IconType.CIRCLE_INFO} />} />
        </div>

        <div className="flex flex-col gap-2">
          <TextField label={'opsIntakeForm.benefitMetrics'} value={benefitMetrics} onChange={setBenefitMetrics} />
          <AssistiveText
            text={t('opsIntakeForm.benefitMetricsAssistiveText') || ''}
            variant={AssistiveTextVariant.Brand}
            iconOverride={<Icon type={IconType.CIRCLE_INFO} />}
          />
        </div>

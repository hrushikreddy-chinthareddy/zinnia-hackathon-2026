...
describe('OpsIntakeForm', () => {
  ...
  it('submit button is disabled when form is invalid', () => {
    ...
    const submitBtn = getByLabelText('submit-ops-form') as HTMLButtonElement;
  ...
  it('submits form when valid and calls metadata & onDislikeReasonChange', async () => {
    ...
    fireEvent.change(getByLabelText('opsIntakeForm.processName'), { target: { value: 'Process 1' } });
    fireEvent.change(getByLabelText('opsIntakeForm.blockOfBusiness'), { target: { value: 'Business Block' } });
    fireEvent.change(getByLabelText('opsIntakeForm.processDescription'), { target: { value: 'Description' } });
    fireEvent.change(getByLabelText('opsIntakeForm.outcomeExpected'), { target: { value: 'Outcome' } });
    fireEvent.change(getByLabelText('opsIntakeForm.smeEmail'), { target: { value: 'sme.email@zinnia.com' } });
    fireEvent.change(getByLabelText('Low'), { target: { value: 'Low' } });
    fireEvent.change(getByLabelText('Daily'), { target: { value: 'Daily' } });
    fireEvent.change(getByLabelText('opsIntakeForm.benefitMetrics'), { target: { value: 'Benefit Metrics' } });

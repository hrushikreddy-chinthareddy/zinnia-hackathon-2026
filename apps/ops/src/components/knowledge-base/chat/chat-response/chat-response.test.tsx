...
describe('ChatResponse', () => {
  ...
  it("renders response with markdown formatting", async () => {
    const { getByText, getByTestId } = render(<ChatResponse {...defaultProps} />);
    await waitFor(() => {
  ...
  it('sends like feedback on clicking upvote', async () => {
    ...
    await waitFor(() => {
      expect(sendResponseFeedback).toHaveBeenCalledWith("response-123", 'test@zinnia.com', FeedbackType.Like);
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Feedback sent successfully');
    });
    jest.useRealTimers();
  });
  ...
  it('cancels like api call if like is deselected before 30sec', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Cancelled Like feedback api call');
    });
  });
  ...
  it('clears the like api timer if feedback comment is submitted before 30 sec', async () => {
    ...
    const submitFeedbackButton = getByRole('button', { name: /submit-feedback-button/i });
    fireEvent.click(submitFeedbackButton);

    const textbox = getByRole('textbox');
    const submitButton = getByRole('button', { name: /submit-feedback-msg/i });
    fireEvent.change(textbox, { target: { value: 'test comment' } });
    fireEvent.click(submitButton);

    await waitFor(() => {
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Cancelled Like feedback api call');
      expect(sendResponseFeedback).toHaveBeenCalledWith("response-123", 'test@zinnia.com', FeedbackType.Like, 'test comment');
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Feedback message sent successfully');
    });
  })
  ...
  it('sends dislike feedback only on clicking downvote and submitting feedback comment', async () => {
    ...
    await waitFor(() => {
      expect(sendResponseFeedback).toHaveBeenCalledWith("response-123", 'test@zinnia.com', FeedbackType.Dislike, 'test comment');
      expect(mockBrowserLogTrace).toHaveBeenCalledWith('Feedback message sent successfully');
    });
  });
  ...
  it('shows error when empty comment is submitted', async () => {
    (sendResponseFeedback as jest.Mock).mockResolvedValue(true);
    const { getByRole, getByText } = render(<ChatResponse {...defaultProps} />);
    const dislikeButton = getByRole('button', { name: /downvote-button/i });
    fireEvent.click(dislikeButton);
  ...
  it('logs errors when fails to send a feedback on clicking upvote', async () => {
    ...
    await waitFor(() => {
      expect(sendResponseFeedback).toHaveBeenCalledWith("response-123", 'test@zinnia.com', FeedbackType.Like);
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error sending feedback ::', { error: testError });
    });
    jest.useRealTimers();
  });
  ...
  it('logs errors when fails to send a feedback comment', async () => {
    ...
    await waitFor(() => {
      expect(sendResponseFeedback).toHaveBeenCalledWith("response-123", 'test@zinnia.com', FeedbackType.Dislike, 'test comment');
      expect(mockBrowserLogError).toHaveBeenCalledWith('Error sending feedback message::', { error: testError });
    });
  });
  ...
  it('closes the feedback comment textbox on clicking cancel when like feedback is submitted', async () => {
    ...
    const cancelButton = getByRole('button', { name: /cancel-feedback-msg/i });
  ...
  it('opens the followup modal when clicking on ask follow up questions button', async () => {
    const { getByRole, getByText } = render(<ChatResponse {...defaultProps} />);
    const followUpButton = getByRole('button', { name: /followup-button/i });
    fireEvent.click(followUpButton);

    await waitFor(() => {
      const modalContent = document.querySelector('[data-testid="followup-modal"]');

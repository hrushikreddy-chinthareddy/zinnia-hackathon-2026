...
const DislikeReasons = ({ dislikeReason, onDislikeReasonChange }: DislikeReasonsProps) => {
  ...
  const dislikeReasonOptions = {
    ...
    relevantDocumentMissing: t('chat.feedback.dislikeReasons.relevantDocumentMissing'),
  ...
  const [metadata, setMetadata] = useState<OpsIntakeFormPayload>({} as OpsIntakeFormPayload);
  ...
  const handleChangeRadio = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newDislikeReasonPayload = {
      reason: newReason,
      ...(
        newReason === (dislikeReasonOptions.incorrectResponse || dislikeReasonOptions.relevantDocumentMissing) ?
          { links: documentLinks } :
          { links: null }
      ),
      ...(
  ...
  const radioOptions = [
      ...
      subElement: (
        <div className="flex flex-col gap-2">
          ...
          </Typography>
          {
            ...
            <Field
              label={t('chat.feedback.dislikeReasons.linksLabel') || ''}
    ...
    },
    {
      ...
      subElement: (
        <div className="flex flex-col gap-2" >
          <Typography variant={TypographyVariant.BodyParagraph}>
            {t('chat.feedback.dislikeReasons.relevantDocumentMissing')}
          </Typography>
          {
            reason === dislikeReasonOptions.relevantDocumentMissing &&
            <Field
              label={t('chat.feedback.dislikeReasons.linksLabel') || ''}
              size={FieldSize.Small}
              type={FieldType.BaseActive}
        ...
        <div className="flex flex-col gap-2 items-start">
          ...
          </Typography>
          {
            ...
            <Button
              ...
              {
                Object.keys(metadata).length > 0 && <Icon type={IconType.CIRCLE_CHECKMARK} className="mr-1" />
              }
  ...
  return (
    <div className="mt-2">
      <Typography variant={TypographyVariant.BodySmBold} className="mb-2">
        {t('chat.feedback.dislikeReasonsHeader')}<span className="text-red-700">*</span>
      </Typography>
      <Radio
        items={radioOptions}
        value={reason}

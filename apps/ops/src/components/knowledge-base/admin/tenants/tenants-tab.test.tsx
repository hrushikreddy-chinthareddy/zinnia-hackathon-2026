...
const mockClient = [
  {
    client: {
      ...
      "title": "Common",
      "thumbnailWebUrl": "https://media.akamai.odsp.cdn.office.net/se2office365.sharepoint.com/_layouts/15/images/sitepagethumbnail.png",
...
describe('TenantsTab', () => {
  ...
  it('shows logs when an error occurs while fetching tenants', async () => {
    (getAllClientDetails as jest.Mock).mockRejectedValue(new Error('Network failure'));
    render(<TenantsTab />);
    await waitFor(() => {
      expect(getAllClientDetails).toHaveBeenCalled();
  ...
  it('sorts result by created and updated date', async () => {
    ...
    const expectedCreatedSorted = [...parsedCreatedDates].sort((a, b) => a - b);
  ...
  it('on clearing the search term it again displays all tenants', async () => {
    (getAllClientDetails as jest.Mock).mockResolvedValue(mockClient);
    const { getByPlaceholderText, getByRole, getByText } = render(<TenantsTab />);
    await waitFor(() => {
      expect(getAllClientDetails).toHaveBeenCalled();
    });
  ...
  it('toggles sort direction when column header is clicked', async () => {
    ...
    const createdDateCellsAfterToggle = await findAllByTestId('tenant-created-date');
    const createdDatesAfterToggle = createdDateCellsAfterToggle.map(cell => cell.textContent!.trim());
    const parsedCreatedDatesAfterToggle = createdDatesAfterToggle.map(date => new Date(date).getTime());
    const expectedSortedAfterToggle = [...parsedCreatedDatesAfterToggle].sort((a, b) => b - a);
    expect(parsedCreatedDatesAfterToggle).toEqual(expectedSortedAfterToggle);
  });

...
import { Divider, Icon, IconType, Loader, Table, TableBody, TableCell, TableHeader, TableHeaderCell, TableRow } from '@zinnia/bloom/components';
...
import { DocumentsDisplayType, KeyboardEvents, SortDirection, SortFields } from '@deps/types/knowledge-base';
...
const DocumentsPage = ({ docs }: DocumentsPageProps) => {
  ...
  const [currentDocument, setCurrentDocument] = useState<ClientDocumentDto | null>(null);
  ...
  const fetchDocuments = useCallback(async (page: number, pageSize = 10) => {
  ...
  const handleGoToPage = useCallback((offset: number) => {
    const page = offset - 1;
    setPage(page);
    ...
    <div className="mt-6">
      {docsToRender.length > 0 ? (
        <>
            ...
            <TableHeader>
              <TableRow>
                <TableHeaderCell
                  ...
                  onClick={() => toggleSortConfig(SortFields.Name)}
                  sortable={true}>
                  {t('documents.fileName')}
                  {
                    sortConfig?.key === SortFields.Name && renderSortIcon()
                  }
                </TableHeaderCell>
                <TableHeaderCell
                  className="typography-content-body-sm-bold"
                  onClick={() => toggleSortConfig(SortFields.Updated)}
                  sortable={true}>
                  {t('documents.updatedAt')}
                  {
                    sortConfig?.key === SortFields.Updated && renderSortIcon()
                  }
                </TableHeaderCell>
                <TableHeaderCell
                  className="typography-content-body-sm-bold"
                  onClick={() => toggleSortConfig(SortFields.Created)}
                  sortable={true}>
                  {t('documents.createdAt')}
                  {
                    sortConfig?.key === SortFields.Created && renderSortIcon()
                  }
                </TableHeaderCell>
              </TableRow>
            </TableHeader>
                  ...
                  <TableRow key={webUrl}>
                    <TableCell
                      ...
                      onClick={() => {
                        ...
                        setCurrentDocument({ name, webUrl, createdAt, updatedAt, itemId, driveId });
                      }}
                      onKeyDown={(e) => {
                        if (e.key === KeyboardEvents.Enter || e.key === KeyboardEvents.Space) {
                          e.preventDefault();
                          setOpenDocumentPreview(true);
                          setCurrentDocument({ name, webUrl, createdAt, updatedAt, itemId, driveId });
                        }
          ...
          <Modal open={openDocumentPreview}
            closeIcon='X'
            ...
            <div className="pt-4 flex gap-8 items-end justify-center">
              {
                docs === DocumentsDisplayType.All && (
                  <div className='flex flex-col gap-1'>
                    <SelectComponent
                      label={t('documents.resultsPerPage') || ''}
              ...
              <PaginationControls limit={pageSize}
          ...
          <div className="flex justify-end my-4">
            <Field
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => {
                ...
                setSearchTerm(term);
                setSearchPage(0);
                if (term.trim().length >= MIN_SEARCH_TERM_LENGTH) {

...
describe('Documents', () => {
    ...
    it('opens a doc modal when enter key is pressed', async () => {
        ...
        fireEvent.keyDown(documentLink, {
            key: 'Enter',
            code: 'Enter',
            charCode: 13,
        });
    ...
    it('sorts result by name, and date', async () => {
        ...
        let updatedCells = await waitFor(() =>
            findAllByTestId('document-updated')
        );
        const updatedAsc = updatedCells.map(
            (cell) => new Date(cell.textContent || '')
        );
        expect(updatedAsc).toEqual(
            [...updatedAsc].sort((a, b) => a.getTime() - b.getTime())
        );

        fireEvent.click(getByText('documents.updatedAt'));
        updatedCells = await waitFor(() => findAllByTestId('document-updated'));
        const updatedDesc = updatedCells.map(
            (cell) => new Date(cell.textContent || '')
        );
        expect(updatedDesc).toEqual(
            [...updatedDesc].sort((a, b) => b.getTime() - a.getTime())
        );
    });
});

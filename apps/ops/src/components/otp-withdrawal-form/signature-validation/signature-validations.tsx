...
import {
    SignatureFieldNames,
    SignaturePartProps,
} from './signature-validation-parts/signature-parts';
...
export default function SignatureValidations({
    ...
    useEffect(() => {
        const newDisplaySignatures = config.filter(
            ({ shouldDisplay = () => true }) => {
        ...
        if (newDisplaySignatures.length === displaySignatures.length) {
            ...
            for (let i = 0; i < newDisplaySignatures.length; ++i) {
                ...
                if (
                    newDisplay.signatureType !== oldDisplay.signatureType ||
                    newDisplay.key !== oldDisplay.key
                ) {
                    shouldUpdate = true;
                }
            }
        ...
        formDataContext.setFormSignature((fs) => {
            return {
                ...
                signatures: fs.signatures.filter((sig) => {
                    ...
                    return newDisplaySignatures.find(
                        (displaySig) => displaySig.signatureType === signType
                    );
                }),
            };
        });
    ...
    return (
        ...
        >
            ...
            {children}
            {displaySignatures.map(
                ({ signatureType, partyRole, fields, bonusField }) => (

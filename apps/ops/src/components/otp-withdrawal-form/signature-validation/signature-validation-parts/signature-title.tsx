...
export default function SignatureTitle({
    ...
    const { errors, signTitle, setSignTitle, signType } = useContext(
    ...
    return (
        <SelectSimple
            ...
            onChange={(val) =>
                setSignTitle({
                    text: val === SignatureDesignation.Unselected ? null : val,
                })
            }
            ...
            value={
                signTitle?.text || (SignatureDesignation.Unselected as string)
            }
            variant={
                errors[SignatureFieldNames.SignatureTitle]
                    ? FieldVariant.Error

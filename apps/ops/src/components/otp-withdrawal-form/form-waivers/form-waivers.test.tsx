...
const config: WaiverItemConfig[] = [
    ...
    { id: PolicyWaiver.TERMINAL_ILLNESS, title: 'Title 2', optionTitle: 'Option 2', options: getStandardOptions('2') },
];
...
describe('FormWaivers Component', () => {
    test('should render required checkboxes for additional waiver', () => {
        ...
        const checkbox1: HTMLInputElement = screen.getByTestId('NURSING_HOME_AND_HOSPITAL-checkbox');
        const checkbox2: HTMLInputElement = screen.getByTestId('TERMINAL_ILLNESS-checkbox');
    ...
    test('should render required button group for checkboxes in additional waiver', () => {
        ...
        const optionYes: HTMLInputElement = screen.getByTestId('button-group-label-test-id-yes-1');
        const optionNo: HTMLInputElement = screen.getByTestId('button-group-label-test-id-no-1');
    ...
    test('should add a first additional waiver item without Yes/No selected', () => {
        ...
        render(
            <FormDataContext.Provider value={{ ...defaultFormDataContext, formAdditionalWaivers, setFormAdditionalWaivers: setMockData }}>
                <FormWaivers config={config} />
            </FormDataContext.Provider>
        );

        const checkbox: HTMLInputElement = screen.getByTestId('TERMINAL_ILLNESS-checkbox');
    ...
    test('should add a first additional waiver item with Yes/No selected', () => {
        ...
        const optionNo: HTMLInputElement = screen.getByTestId('button-group-label-test-id-yes-2');
        expect(optionNo).toBeInTheDocument();
    ...
    test('should update additional waiver item for option (Yes/No) value', () => {
        ...
        const optionNo: HTMLInputElement = screen.getByTestId('button-group-label-test-id-no-2');
        expect(optionNo).toBeInTheDocument();
        fireEvent.click(optionNo);

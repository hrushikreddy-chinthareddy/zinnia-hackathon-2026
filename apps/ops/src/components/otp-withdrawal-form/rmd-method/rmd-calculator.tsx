...
const calculateAgeDifference = (ownerAge: string, spouseAgee: string): number => {
...
export default function RMDCalculator({ isFormStateReadOnly }: RMDCalculatorProps) {
    ...
    const { contractStatus, qualType } = useAccountInfo(initialForm?.data?.contractNum, initialForm.carrier as string);
    ...
    useEffect(() => {
        ...
        const filteredAnnuitants = annuitants?.map(party => {
            const annuitant: RmdParty = {
                partyRole: party?.SrcRole?.toLowerCase().includes(RmdRoles.Insured.toLowerCase())
                    ? RmdRoles.Insured
                ...
                try {
                    ...
                    const query: CalculateRmdBody = {
                        policy: {
                            ...
                            QualificationType: qualType == QualTypes.NonQualified ? RmdQualTypes.NonQualified : RmdQualTypes.Qualified,
                    ...
                    setRmdFactor(response.items.calculatedValues[0].duration || 0); // rmdFactor
                    response.items.calculatedValues[0].rmdBasis && setPriorYearMrdBasisValue(response.items.calculatedValues[0].rmdBasis); // MRD Basis value
                    setLoader(false);
                } catch (e: any) {
                    setRmdFactor(0);
                    setLoader(false);
        ...
        const getRMDBasisValue = async (rmdYear: string) => {
            try {
                ...
                const mrdBasis = response?.variableQuoteArray?.find(
                    variableQuote => variableQuote.variableDescription == VariableQuoteDescription.MRDBasis
                )?.variableValue;
                ...
                setApiError(e?.message as string);
                console.error('GetRMDBasisValue::Error retrieving variable quote api', e);
            }
        };

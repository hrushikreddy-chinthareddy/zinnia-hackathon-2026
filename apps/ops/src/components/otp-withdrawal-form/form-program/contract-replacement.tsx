...
export default function ContractReplacement({ isFormStateReadOnly }: ContractReplacementProps) {
    const { formProgram, setFormProgram } = useContext(FormDataContext);
    const { t } = useTranslation(undefined, { keyPrefix: 'caseWithdrawal.request.amountDetails.contractReplacement' });
    const [isContractReplaced, setIsContractReplaced] = useState(formProgram?.isContractReplaced?.text || false);

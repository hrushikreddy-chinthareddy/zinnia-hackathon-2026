...
import { CaseStatus, ProcessRequestType } from '@deps/models/case/withdrawal/case';
import { ZAHARA_API_DATE_FORMAT } from '@deps/types/constants';
import { FEATURE_FLAGS } from '@deps/utils/optimizely/flags';

const AsOfDateComponent = () => {
    const { formProgram, currentFormState, setFormProgram, featureFlagDecisions } = useContext(FormDataContext);
    const searchParams = useSearchParams();

    const processRequestType = formProgram?.processRequestType?.[0]?.text || null;
    const { t } = useTranslation(undefined, { keyPrefix: 'caseWithdrawal.request.amountDetails.processTimeframe' });
    const [asOfDate, setAsOfDate] = useState(
        formProgram?.asOfDate?.text && dayjs(formProgram.asOfDate?.text, ZAHARA_API_DATE_FORMAT).isValid()
            ...
            return {
                ...
                asOfDate: {
                    text:
                        ...
                        ogFormProgran?.processRequestType?.[0]?.text === ProcessRequestType.AsOfDate &&
                        asOfDate
                            ? dayjs(asOfDate, DATE_PICKER_FORMAT).format(ZAHARA_API_DATE_FORMAT)
                            : null,
                },
            };
        });
    ...
    return (
        <div className={`-mt-4 flex flex-row items-center gap-2`}>
            <Content contentClassName="h-14 flex items-center" variant={ContentVariant.BodySm} details={t('asOfThisDate') as string} />
            {formProgram?.processRequestType?.[0]?.text === ProcessRequestType.AsOfDate && (
                <FieldDateSelect

...
} from '../../signature-validation/signature-validation';
...
export const getDisbursmentConsent = (
    consentData: DisbursmentConsentInfo | undefined
) => (consentData?.isConsent.text ? consentData : defaultDisbursmentConsent);

export const ConsentAvailable = ({
    signatureFields,
    isFormStateReadOnly,
}: ConsentAvailableProps) => {
    const { initialForm, formDisbursement, setFormDisbursement } =
        useContext(FormDataContext);
    const disbursementConsent =
        initialForm?.data?.formRequest?.formDisbursement?.disbursmentConsent ??
        defaultDisbursmentConsent;
    const { t } = useTranslation(undefined, {
        keyPrefix: 'caseWithdrawal.request.distributionMethod',
    });
    const [isConsent] = useState(
        formDisbursement?.disbursmentConsent?.isConsent.text ?? null
    );
    const initialSignature = {
        ...
        signOtherTitle: null,
        signTitle: {
            text: disbursementConsent.signTitle.text as string | null,
        },
        signTitles: [{ text: null }] as TextTuple,
        signType: {
            text: SignatureValidationTypeWithdrawal.Owner as SignatureValidationTypeWithdrawal | null,
        },
    ...
    useEffect(() => {
        setFormDisbursement((ogFormDisbursement) => ({
            ...
            disbursmentConsent: {
                ...
                signTitle: {
                    text:
                        isConsent && signature.signTitle.text
                            ? signature.signTitle.text
                            : '',
                },
                signDate: {
                    text:
    ...
    return (
        <div
            className={`my-4 flex flex-col gap-4 md:grid md:grid-cols-2 md:grid-rows-2 lg:grid-cols-auto-2 lg:grid-rows-1`}
        >
            <Field

...
export function usePartyFields(party: Party, formErrors?: FormValidationErrors) {
    const { t } = useTranslation(undefined, { keyPrefix: 'caseWithdrawal.request.personalDetails' });
    const ssnFormat = { format: '#########' };

    const formDob = party?.dob?.text ? dayjs(party?.dob?.text, ZAHARA_API_DATE_FORMAT).format(DATE_PICKER_FORMAT) : '';
    ...
    const [maritalStatus, setMaritalStatus] = useState(party?.maritalStatus?.text || '');
    ...
    const middleNameField = ({label, isFormStateReadOnly}: IFieldConfig) => (
        <Field
            ...
            type={FieldType.BaseActive}
            value={middleName}
            variant={selectVarientByConfig({ value: middleName, isFormStateReadOnly })}
    ...
    const lastNameField = ({label, isFormStateReadOnly}: IFieldConfig) => (
        <Field
            ...
            type={FieldType.BaseActive}
            value={lastName}
            variant={selectVarientByConfig({ value: lastName, isFormStateReadOnly })}
        />
    );
    ...
    const renderField = (field: PartyFieldConfig, isFormStateReadOnly?: boolean): JSX.Element | null => {
        switch (field.fieldName) {
            ...
            case PartyFields.Email:
                return <div key={field.fieldName}>{emailField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
            case PartyFields.MaritalStatus:
                return <div key={field.fieldName}>{maritalStatusField({ label: field.fieldLabel, isFormStateReadOnly })}</div>;
...
export function SingleParty({ fields, formErrors, formParty, isFormStateReadOnly, onDataChange }: SinglePartyProps) {
    ...
    return <div className="my-4 grid w-full grid-cols-3 gap-2">{fields?.map((field) => renderField(field, isFormStateReadOnly))}</div>;
}

...
describe('Formparty component', () => {
    describe('FLIC Form', () => {
        it('should render empty formControls if parties object is empty', async () => {
            ...
            const addressLine1Input =
                screen.getAllByLabelText('mailingAddress')[0];
        ...
        it.skip('should render formControls based on configuration configuration', async () => {
            ...
            render(
                <FormDataContext.Provider
                    value={{
                        ...defaultFormDataContext,
                        formParty: mockformPartyData,
                        setFormParty: setMockData,
                    }}
                >
                    <FormParties configs={formPartyConfigs} />
                </FormDataContext.Provider>
        ...
        it.skip('should render addressLine2 if available', () => {
            CaseDetails.data.formRequest.formParty.parties[0].addresses[0].addressLine2 =
                'sierra2';
            const mockformPartyData = CaseDetails.data.formRequest.formParty;
            let setMethodArgs;
            const setMockData = jest.fn((cb) => {
            ...
            const addressLine2Input = screen.getAllByLabelText(
                'mailingAddressLine2'
    ...
    describe('SBGC Form', () => {
        it('should render empty formControls if parties object is empty', async () => {
            ...
            // Phone Fields
            const ownerPhoneDayInput =
                screen.getAllByLabelText('daytimePhone')[0];
            expect(ownerPhoneDayInput).toBeInTheDocument();

            const OwnerPhoneHomeInput =
                screen.getAllByLabelText('daytimePhone')[1];
    ...
    describe('DLIC Withdrawal Form', () => {
        it('should render empty party formControls if parties object is empty', () => {
            ...
            // Address Fields
            const addressHasChanged = screen.getAllByLabelText(
    ...
    describe('OFT FLIC Form', () => {
        it.skip('Should not render Address controls for flic withdrawal', () => {
            const { formPartyConfigs } = getFlicOftConfig(
                t,
                QualTypes.CustInhIRA
            );
            let setMethodArgs;
            const mockformPartyData = CaseDetails.data.formRequest.formParty;
            const setMockData = jest.fn((cb) => {
            ...
            const addressLine1Input = screen.queryByLabelText(
                'mailingAddress'
            ) as HTMLInputElement;
            const cityInput = screen.queryByLabelText(
                'city'
            ) as HTMLInputElement;
            const stateInput = screen.queryByLabelText(
                'state'
            ) as HTMLInputElement;

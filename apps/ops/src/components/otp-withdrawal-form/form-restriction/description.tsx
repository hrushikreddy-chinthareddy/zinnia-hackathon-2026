...
export function Description() {
    ...
    const [description, setDescription] = useState(getDescription(formRestriction?.emergency ?? []));

    const isBeyondControlOptionSelected = formRestriction.emergency.some(reason => reason.text === EmergencyOption.BeyondControl) || null;
            ...
            return {
                ...
                emergency: ogFormRestriction?.emergency.map(restriction => {
                    if (restriction.text === EmergencyOption.BeyondControl) {
                        restriction.selectionOptions.DistribUnforseenDesc = { text: description };
                    }
                    return restriction;
                }),
            };
    ...
    return (
        <>
            {formRestriction?.emergency?.some(emergency => emergency.text === EmergencyOption.BeyondControl) && (
                <div className="ml-8">
                    <div className="mb-2">
                        <Typography variant={TypographyVariant.Label}>{t('detailedDescription')}</Typography>
                    </div>
                    <div

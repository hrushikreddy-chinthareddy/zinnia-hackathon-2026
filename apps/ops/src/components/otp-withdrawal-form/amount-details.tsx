...
export default function AmountDetails({
    ...
    const [programType, setProgramType] = useState(
        determineProgramType(formProgram)
    );
    const [amount, setAmount] = useState(
        formProgram?.partialAmount?.text || ''
    );
    ...
    useEffect(() => {
        ...
        const programSubTypeForForm: FormProgram['programSubType'] = {
            text: null,
        };
        const partialAmountForForm: FormProgram['partialAmount'] = {
            text: null,
            amountType: AmountType.Dollar,
        };
        const gmwbAmount: FormProgram['gmwbAmount'] = {
            text: null,
        ...
        if (isOnlyWithdrawalTypeControls) {
            setFormProgram((fs) => ({
                ...fs,
                withdrawType: {
                    text: withdrawType ? withdrawType : WithdrawalType.Gross,
                },
            }));
        } else {
            setFormProgram({
                ...
                asOfDate: effectiveDate
                    ? {
                          text: dayjs(effectiveDate, DATE_PICKER_FORMAT).format(

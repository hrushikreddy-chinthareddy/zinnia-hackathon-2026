...
export const getNotificationStatusText = (
    ...
    t: TFunction
): { notificationIcon: React.ReactNode; notificationText: string; dateText: string } => {
    const { send, receive, sendDateTime, scheduleDateTime, deliveryMethod } = notification;
    let dateText = '';
    let timestamp: string | undefined;
    const translatedMethod = t(`caseOverview.notifications.types.${deliveryMethod.toLowerCase()}`);
    ...
    if (send === false && receive === false) {
        ...
        dateText = timestamp
            ...
            : '';
        return {
            notificationText: t('caseOverview.notifications.scheduledNotification', {
                method: translatedMethod,
                followup: notification.followupId,
            }),
            notificationIcon: <NotStartedIcon className="text-gray-300" width={16} height={16} />,
            dateText,
        };
    ...
    const displayNotification = (notification: any) => {
        switch (notification.deliveryMethod) {
            ...
            case DeliveryMethods.Faxnumber:
                return (
                    <div>
                        <Label>{t('caseOverview.notifications.types.faxnumber')}</Label>
...
export function BeneSideSheetStep({ step }: { step: TransformedStep }) {
    ...
    const [hasReceivedNotification, setHasReceivedNotification] = useState(false);
    ...
    useEffect(() => {
        const fetchTransactions = async (identifier: string) => {
            ...
            let transactions: INotification[] = transactionResponse?.entity?.followupDetails || [];
            transactions = transactions.filter(transaction => transaction.followupStatus !== NotificationStatus.Canceled);
            if (transactions.length > 0) {
                transactions.sort((transaction1, transaction2) => {
                    const date1 = new Date(transaction1.sendDateTime || transaction1.scheduleDateTime || transaction1.statusDateTime || 0);
                    const date2 = new Date(transaction2.sendDateTime || transaction2.scheduleDateTime || transaction2.statusDateTime || 0);
                    return date1.getTime() - date2.getTime();
                });

                const received = transactions.some(transaction =>transaction.followupStatus === NotificationStatus.Receive);
                setHasReceivedNotification(received);

                const hasExceptionOrNigo = transactions.some(
                    transaction => transaction.followupStatus === NotificationStatus.Exception || transaction.followupStatus === NotificationStatus.NIGO
                );
            ...
            if (transactionResponse?.identifiers && transactionResponse?.identifiers?.length > 0) {
                const policyIdentifier = transactionResponse?.identifiers.find((item: { identifier: string; }) => item.identifier === 'policyNumber');
                if (policyIdentifier) {
    ...
    const findNextResetNotification = (currentIndex: number): INotification | null => {
            ...
            <div>
                {
                    ...
                    const { notificationIcon, notificationText, dateText } = getNotificationStatusText(notification, t);
                    const flowstart = t('caseOverview.notifications.startNotification');
                    const resendText = t('caseOverview.notifications.resendNotification', {
                        action: NotificationStatus.Resend.toLowerCase(),
                    });
                    const updateDate = notification?.statusDateTime? t('caseOverview.notifications.updatedStatusTooltipWithDate', {date: formatTimestamp(notification.statusDateTime),}): '';
                    const resetText = t('caseOverview.notifications.resetNotification', {
                    ...
                    const nextResetNotification = notification.followupStatus === NotificationStatus.Reset ? findNextResetNotification(index) : null;
                    return (
                        <div key={`notification-wrapper-${index}`}>
                            {index === 0 && (
                                <div className="flex flex-row gap-2 mb-4" key={`notification-${index}${index}`}>
                                    <div className="flex h-6 w-6 shrink-0 items-center justify-center">
                                        <CircleInfoIcon className="text-semantic-info" width={16} height={16} />
                                    </div>
                                    <div className="flex w-full flex-col gap-1">
                                        <Content variant={ContentVariant.BodySm} details={flowstart} />
                                        <SidesheetNotification notification={notification} />
                                        <Content className="text-gray-600" variant={ContentVariant.BodySm} details={dateText} />
                                    </div>
                                </div>
                            )}

                            {notification.followupId !== 5 && (
                                <div className="flex flex-row gap-2 mb-4" key={`receive-notification-${index}`}>
                            ...
                            {notification.followupStatus === NotificationStatus.Resend && (
                                <div className="flex flex-row gap-2 mb-4" key={`receive-notification-${index}${index}`}>
                                    <div className="flex h-6 w-6 shrink-0 items-center justify-center">
                                    <EditAlt className="text-blue" width={16} height={16} />
                                    </div>
                                    <div className="flex w-full flex-col gap-1">
                                        <Content variant={ContentVariant.BodySm} details={resendText} />
                                        <Content className="text-gray-600" variant={ContentVariant.BodySm} details={dateText} />
                                    </div>
                                </div>
                            )}
                            ...
                            {notification.followupStatus === NotificationStatus.Reset && (
                                <div className="flex flex-row gap-2 mb-4" key={`receive-notification-${index}${index}`}>
                                    ...
                                    <div className="flex w-full flex-col gap-1">
                                        <Content variant={ContentVariant.BodySm} details={resetText} />
                                        {nextResetNotification && <SidesheetNotification notification={nextResetNotification} />}
                                        <Content className="text-gray-600" variant={ContentVariant.BodySm} details={updateDate} />
                                    </div>
                                </div>
                            )}
                ...
                {!shouldHideButton && (
                <div className='mt-4'>
                    <Button size="small" onClick={() => router.push(`/claims/update-notification-method?policyNumber=${policyNumber}&carrier=${carrier}&recordId=${identifier}`)}>
                        {t('updateNotificationMethodForBeneficiary.updateNotificationMethodStep.title')}
                    </Button>
                </div>
                )}
            </div>

        ...
        case 'RESOLVED' as Statuses:
            return {
                text: t(
                    ...
                    { date: formatTimestamp(step.updatedAt, 'tooltip') }
                ),
                icon: (
                    <CompletedIcon
    ...
    return (
        <div
            className={`flex flex-row items-center justify-start ${
                task.description ? 'gap-1' : ''
            } `}
        >
            <Content
                variant={ContentVariant.BodySm}
                details={task.taskName || task.description}
            />
...
export default function MultiInstanceTab({
    step,
    ...rest
}: { step: TransformedStep } & React.HTMLAttributes<HTMLDivElement>) {
    const { t } = useTranslation();

    const getSideSheetStep = (

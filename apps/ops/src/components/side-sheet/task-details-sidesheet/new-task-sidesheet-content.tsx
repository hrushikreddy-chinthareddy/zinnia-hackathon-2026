...
export default function NewTaskSideSheet({ taskId }: { taskId: string }) {
    ...
    useEffect(() => {
        const getTaskData = async () => {
            ...
            return new Promise<ManagementTask | null>((resolve) =>
                resolve(taskData)
            );
        };
        ...
        const getData = async () => {
            setLoading(true);
            const [taskData, featureFlagsData] = await Promise.all([
                getTaskData(),
                getFeatureFlagDescisions(),
            ]);
            setTask(taskData);
    ...
    const { unit: createdUnit, count: createdCount } =
        getTimeAgoUnitValue(task.createdAt) || {};
    const formattedCreated = parseAndFormatDate(
        NUMERIC_DATE_FORMAT,
        DEFAULT_DATE_FORMAT,
        task.createdAt
    );
        ...
        <div className="flex flex-col w-full">
            ...
            <div className="flex flex-col items-start justify-center gap-1 border-b-2 border-gray-100 py-4 last:border-b-0">
                <div className="flex flex-row items-center gap-1">
                    <Label
                        label={t('sideSheet.task.status.label')}
                ...
                <div className="flex flex-row items-center gap-1">
                    <Label
                        label={t('sideSheet.task.assigneeLabel')}
                        variant={LabelVariant.FieldLabel}
                        className="w-[100px] py-2"
                    />
                ...
                <div className="flex flex-row items-center gap-1">
                    <Label
                        label={t('sideSheet.task.createdLabel')}
                        variant={LabelVariant.FieldLabel}
                        className="w-[100px] py-2"
                    />
                    <Typography
                ...
                <div className="flex flex-row items-center gap-1">
                    <Label
                        label={t('sideSheet.task.stepLabel')}
                        variant={LabelVariant.FieldLabel}
                        className="w-[100px] py-2"
                    />
                    <Typography
                ...
                <div className="flex flex-row items-center gap-1">
                    <Label
                        label={t('sideSheet.task.detailsLabel')}
                        variant={LabelVariant.FieldLabel}
                        className="w-[100px] py-2"
                    />
                    <div className="pl-14">
    ...
    const renderTabContent = (
        <>
            <TabContent
                className="flex px-10 flex-col items-center"
        ...
        <div>
            ...
            >
                <TabList className="!mb-0 w-full px-4 pt-4 md:px-6 lg:px-8">
                    ...
                    <TabTrigger value={TabOptions.History}>
                        {t('sideSheet.task.tabs.history') ?? ''}
                    </TabTrigger>

...
const WithdrawalApiErrorState = ({ onCancel, onContinue, arrangementType }: ApiErrorStateProps) => {
    const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'transactions.states.apiErrorWithdrawal' });

    const subtitle = (
        <>
            {t('subtitle1')}
            <Typography variant={TypographyVariant.BodyBold} className="inline">
                {arrangementType === ArrangementType.WITHDRAWAL ? t('subtitleWithdrawal') : t('subtitleRmd')}
            </Typography>
            {t('subtitle2')}

...
export default function SidesheetReverseRecreate({
    ...
    const [showConfirmSelectionError, setShowConfirmSelectionError] =
        useState(false);

    const [caseDocumentOptions, setCaseDocumentOptions] = useState<
        CaseDocumentOption[]
    >([]);
    const [documentViewState, setDocumentViewState] = useState(
        ViewState.Default
    );
    ...
    const reverseRecreate = async () => {
        ...
        const result = await reverseRecreateTransaction(
            planCode,
            policyNumber,
            correlationId,
            reversalTransactionId,
        ...
        segmentAnalyticsTrackEvent<TransactionContinueClickedEvent>(
            SegmentTrackedEventName.TransactionContinueClicked,
            {
                ...
                userId: partyId,
                type: 'ReverseRecreate' as TransactionTypeEnum,
    ...
    const handleReverseRecreate = () => {
        if (!caseId) {
            setCurrentErrors({
                caseId: t('workflows.start.missingSelection') as string,
            });
        }
        if (!checked) {
        ...
        case ViewState.ApiError:
            return (
                <CardInfo
                    className="mt-8"
                    cta={{
                        action: reverseRecreate,
                        text: t(
                            'policy.history.reverseRecreateSidesheet.reversePremiumPayment'
                        ),
                    }}
                    icon={
                        <HexExclamationIcon
                    ...
                    subtitle={
                        <>
                            {t(
                                'policy.history.reverseRecreateSidesheet.apiError.subtitle'
                            )}
                            <NavElement
                                href={HELP_DESK_LINK}
                                target="_blank"
                    ...
                    title={t(
                        'policy.history.reverseRecreateSidesheet.apiError.title'
                    )}
                />
            );
            ...
            return (
                <div className="m-8">
                    ...
                    <div className="mb-8 mt-8 flex gap-8">
                        <FieldData
                            label={t(
                                'policy.history.reverseRecreateSidesheet.originalAppliedAmount'
                            )}
                            tooltipBody={t(
                                'policy.history.reverseRecreateSidesheet.appliedAmountBody'
                            )}
                            tooltipTitle={t(
                                'policy.history.reverseRecreateSidesheet.appliedAmountTitle'
                            )}
                        >
                            {amountString}
                        </FieldData>
                        <FieldData
                            label={t(
                                'policy.history.reverseRecreateSidesheet.effectiveDate'
                            )}
                            tooltipBody={t(
                                'policy.history.reverseRecreateSidesheet.effectiveDateBody'
                            )}
                            tooltipTitle={t(
                                'policy.history.reverseRecreateSidesheet.effectiveDate'
                            )}
                    ...
                    <CheckboxText
                        label={t(
                            'policy.history.reverseRecreateSidesheet.confirmationCheckbox',
                            {
                                amount: amountString,
                    ...
                    {showConfirmSelectionError && (
                        <AssistiveText
                            ...
                            text={t(
                                'policy.history.reverseRecreateSidesheet.alert'
                            )}
                        />
                    )}
                    <div className="mt-10 flex items-center gap-8">
                        <Button
                            onClick={handleReverseRecreate}
                        ...
                        >
                            {t(
                                'policy.history.reverseRecreateSidesheet.cancel'
                            )}
                        </NavElement>
                    </div>
                </div>
            ...
            return (
                <CardInfo
                    ...
                    subtitle={
                        <>
                            {t(
                                'policy.history.reverseRecreateSidesheet.successSubtitle'
                            )}
                            <span className="font-bold">
                                {t(
                                    `policy.history.reverseRecreateSidesheet.transactionTypes.${transactionType}`
                                )}
                            </span>
                            {t(
                                'policy.history.reverseRecreateSidesheet.successSubtitle2'
                            )}
                            <span className="font-bold">{amountString}</span>
                            {t(
                                'policy.history.reverseRecreateSidesheet.successSubtitle3'
                            )}
                            <span className="font-bold">{'$0.00'}</span>
                            {t(
                                'policy.history.reverseRecreateSidesheet.successSubtitle4'
                            )}
                            <span className="font-bold">{effectiveDate}</span>
                            {t(
                                'policy.history.reverseRecreateSidesheet.successSubtitle5'
                            )}
                        </>
                    }
                    title={t(
                        'policy.history.reverseRecreateSidesheet.successTitle'
                    )}
                />
            );
        default:

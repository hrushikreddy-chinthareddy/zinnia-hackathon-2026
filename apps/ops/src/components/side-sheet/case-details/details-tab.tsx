...
function DetailsTab({
    ...
    const formattedIssueDate = policy?.policyDates?.issueDate
        ? dayjs(policy?.policyDates?.issueDate).format(
              DEFAULT_EXTENDED_DATE_FORMAT
          )
        : DEFAULT_ERROR_STRING;
    const formattedApplicationDate = policy?.policyDates?.applicationDate
        ? dayjs(policy?.policyDates?.applicationDate).format(
              DEFAULT_EXTENDED_DATE_FORMAT
          )
        : DEFAULT_ERROR_STRING;
    const formattedContractValue = formatCurrencyLocal(
        documentData.contractValue as string
    );
            ...
            <div className="flex float-start">
                ...
                <div>
                    ...
                    <div className="text-md">
                        {documentData?.productName || DEFAULT_ERROR_STRING}
                    </div>
                    <div className="text-md">
                        Contract #: {policyNumber || DEFAULT_ERROR_STRING}
                    </div>
                </div>
            </div>

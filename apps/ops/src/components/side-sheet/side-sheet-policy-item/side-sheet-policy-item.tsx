...
function PolicyHeader({ t }: { t: TFunction }, { policy }: SideSheetPolicyItemProps) {
    // ToDo: BPB - move on up
    const totalMinRequiredAmount = getTotalMinRequiredAmount(new PolicyDetails(policy));
...
function PolicySummary({ policySummaryLists, titles }: { policySummaryLists: JSX.Element; titles: string[] }) {
    return (
        <div className="pt-6">
            <CardColumns items={[policySummaryLists]} titles={titles} variant={CardColumnsVariant.SIDE_SHEET} />
        </div>
    );
}

function PolicyPeople({ policyOwnerDto }: { policyOwnerDto: PolicyOwnerDto[] }) {
    return (
        <div className="pb-6 pt-12">
            <p className="leading-7.5 mb-4 font-primary text-xl font-medium text-gray-900">People</p>
            <DepTable cols={policyPartyColumns} rowData={policyOwnerDto} automaticHeight={true} />
        </div>
    );
}
...
function PolicyAction() {
    return (
        <div className="border-t-solid flex items-center justify-center gap-1.5 border-t-2 border-t-gray-100 px-[133.5px] py-4">
            <CirclePlusIcon width={24} height={24} className="shrink-0 text-secondary" />
            <p className="whitespace-nowrap font-primary text-base font-semibold leading-6 text-secondary">Add a contingent beneficiary</p>
        </div>
    );
}

export default function SideSheetPolicyItemForStoryBook({ policy }: SideSheetPolicyItemProps) {
    ...
    const policySummaryInfo = fillColDefs<PolicySummaryColDto>(policySummaryDto, getPolicySummaryColDefs(t), t, 'colDefs:policySummary');
    const policySummaryLists = (
        <DescriptionLists data={policySummaryInfo} labelClassName="text-gray-900" valueClassName="text-gray-900" gap="2.5" />
    );

    const { parties } = policy;
    const policyOwnerDto = parties?.map(party => toPolicyOwnerDto(party)) || ([] as PolicyOwnerDto[]);
    ...
    return (
        <div className="rounded bg-white shadow-elevation-light-04">
            <div className="px-6">
                <PolicyHeader t={t} />
                <PolicySummary policySummaryLists={policySummaryLists} titles={titles} />

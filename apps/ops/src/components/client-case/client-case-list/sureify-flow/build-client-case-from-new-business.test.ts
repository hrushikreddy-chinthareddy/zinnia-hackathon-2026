...
describe('buildClientCaseFromNewBusiness', () => {
    ...
    it('excludes agentDetails and agencyId when excludeAgentUpdate is true', async () => {
        const clientCasePayload = await buildClientCaseFromNewBusiness(
            {
                caseId: 'case-id',
                parties: [
        ...
        expect(clientCasePayload).not.toHaveProperty('agentDetails');
        expect(clientCasePayload).not.toHaveProperty('agencyId');
        expect(getAgencyIdFromHierarchyMock).not.toHaveBeenCalled();
    });
});

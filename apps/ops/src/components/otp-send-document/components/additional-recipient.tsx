...
const AdditionalRecipient = ({ classNames, emails, setEmails, setError, policy }: AdditionalRecipientProps) => {
    ...
    const roleTypes = useMemo(() => (checkCustodialContract ? [PartyRole.INSURED] : AllowedRoleTypesEmail), [checkCustodialContract]);
    ...
    const partyCardsData: PartyAddressCard[] = useMemo(
        () => partyCardsEmail(extractedPartyRoles, extractedParties, qualificationType, addressChangeT),
    ...
    const addEmail = (val: string) => {
        ...
        if (emails.length >= 5) {
            setError(error => ({ ...error, submit: t('errors.maxEmails') as string }));
            return;
        }
        if (emailError) {

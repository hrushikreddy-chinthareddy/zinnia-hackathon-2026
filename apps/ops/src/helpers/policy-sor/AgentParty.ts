...
export const transformAgentDataToParty = (
    ...
    const party: Parties = {
        ...
        addresses: (agentData?.addresses ?? []).map(
            (address: AgentData['addresses'][number]): Address => {
                return {
                    ...
                    addressType:
                        (address.addressType as AddressType) || undefined,
                };
            }
        ),
        ...
        emails: (agentData?.emails ?? []).map(
            (email: AgentData['emails'][number]): Email => {
                return {
        ...
        phones: (agentData?.phones ?? []).map(
            (phone: AgentData['phones'][number]): Phone => {
                return {
                    ...
                    phoneType:
                        (phone.phoneType as Phone['phoneType']) || undefined,

...
export class Coverage {
    ...
    constructor({ coverage = {} }: Policy = {}) {
        ...
        coverage?.coverageLayers?.forEach((coverageLayer) => {
            if (coverageLayer.coverageId) {
                this.coverageLayerById[coverageLayer.coverageId] =
                    coverageLayer;
            }
            if (coverageLayer.coverageType) {
                this.coverageLayerByType[coverageLayer.coverageType] =
                    coverageLayer;
            }
            ...
            if (coverageLayer.coverageParticipants) {
                coverageLayer.coverageParticipants.forEach(
                    (coverageParticipant) => {
                        this.allCoverageParticipants.push(coverageParticipant);
                        if (coverageParticipant.partyId) {
                            this.coverageParticipantByPartyId[
                                coverageParticipant.partyId
                            ] = coverageParticipant;
                        }
                    }
                );
    ...
    public getCoverageLayerById(
        id: string | undefined
    ): CoverageLayer | undefined {
        if (!id) {
            return;
        }
    ...
    public getCoverageLayerByType(
        coverageType: RiderType | undefined
    ): CoverageLayer | undefined {
        if (!coverageType) {
            return undefined;
        }
    ...
    public getCoverageParticipantByPartyId(
        partyId: string | undefined
    ): CoverageParticipants | undefined {
        if (!partyId) {
            return;
        }

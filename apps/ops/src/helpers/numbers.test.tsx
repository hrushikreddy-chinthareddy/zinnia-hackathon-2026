...
describe('Numbers Helper', () => {
    ...
    describe('negativeNumberFormatify', () => {
        it('should convert positive number to string with two decimal places', () => {
            const result = negativeNumberFormatify(123.456);
            expect(result).toBe('($123.46)');
        });

        it('should return $ formatted negative string of negative number with two decimal places when value is negative', () => {
            const result = negativeNumberFormatify(-123.456);
            expect(result).toBe('$-123.46');
        });

        it('should return "$0.00" when input is zero', () => {
            const result = negativeNumberFormatify(0);
            expect(result).toBe('$0.00');
        });
    });

    describe('forcePositiveNumber', () => {
        it('should return string representation of positive number when input is a positive number', () => {
            const result = forcePositiveNumber(42);
            expect(result).toBe('42');
        });

        it('should return default error string when input is a non-numeric string', () => {
            const result = forcePositiveNumber('abc');
            expect(result).toBe(DEFAULT_ERROR_STRING);
        });

        it('should return positive string representation when input is a negative number', () => {
            const result = forcePositiveNumber(-42);
            expect(result).toBe('42');
        });

        it('should return positive string when input is negative string representation of a number', () => {
            const result = forcePositiveNumber('-42');
            expect(result).toBe('42');
        });
    });

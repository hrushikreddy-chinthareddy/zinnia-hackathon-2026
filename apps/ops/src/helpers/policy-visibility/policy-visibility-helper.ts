...
import { planCodeProductOverrides,productTypeVisibilityRules } from '../../product-config/carrier-product-config';
import { DetailLinkType} from '../../types/policy-visibility';

type FeatureKey = 'loans' | 'withdrawals' | 'fundsAndAccounts';

const featureMap: Record<FeatureKey, keyof NonNullable<(typeof productTypeVisibilityRules)[ProductType]>> = {
  loans: 'allowsLoans',
  withdrawals: 'allowsWithdrawals',
  fundsAndAccounts: 'allowsFundsAndAccounts',
};

export const getPolicyVisibility = async (policy: PolicyDetails): Promise<{
  ...
  return {
    ...
    detailLinkType: policy.isAnnuity ? DetailLinkType.Contract : DetailLinkType.Policy,
  };
};
...
const LoansPageHeaderContainer = ({
    policy,
    loanCarryingBalance,
}: LoansContainerProps) => {
    const { t } = useTranslation(TranslationFiles.COMMON);

    const [availableLoanInterestRate, setAvailableLoanInterestRate] = useState(
        t('general.loadingThing', { thing: t('general.rate') })
    );
    const [availableLoanCreditRate, setAvailableLoanCreditRate] = useState(
        t('general.loadingThing', { thing: t('general.rate') })
    );
    ...
    useEffect(() => {
        const getLoanRates = async () => {
            ...
            setAvailableLoanCreditRate(
                percentFormatify(loanCreditRate, { isInteger: true })
            );
            setAvailableLoanInterestRate(
                percentFormatify(loanInterestRate, { isInteger: true })
            );
        };
    ...
    const totalLoanBalanceValue = !isNullEmptyOrUndefined(
        loanValues?.totalLoanBalance
    )
        ? numberFormatify(loanValues?.totalLoanBalance, currencyFormat)
        : DEFAULT_ERROR_STRING;
    const eligibleForLoanValue = !isNullEmptyOrUndefined(
        loanValues?.maximumLoanAmount
    )
        ? numberFormatify(loanValues?.maximumLoanAmount, currencyFormat)
        : DEFAULT_ERROR_STRING;
    const totalNumberOfLoans = !isNullEmptyOrUndefined(
        loanValues?.totalNumberOfLoan
    )
        ? loanValues?.totalNumberOfLoan
        : 0;
    const estimatedNetDeathBenefit = coverage?.netDeathBenefit
        ? numberFormatify(coverage?.netDeathBenefit)
        : DEFAULT_ERROR_STRING;

    const headerRowFlexClassNames = 'mb-4';
    const groupOneFlexClassNames = 'flex gap-4';

    const { data: newLoanEligibility, isLoading: isLoadingNewLoanEligibility } =
        useQuery({
            queryKey: [
    ...
    const belowHeaderTextChildren = (
        <>
            <div className="grid grid-cols-[repeat(2,minmax(min-content,max-content))] gap-x-8 gap-y-4 md:grid-cols-[repeat(3,minmax(min-content,max-content))] md:gap-y-6 lg:flex lg:flex-wrap">
                <FieldData
                    label={t('pageHeader.loans.fields.totalLoans')}
                    variant={FieldDataVariant.Large}
                >
                    {`${totalNumberOfLoans} ${t(
                        'pageHeader.loans.fields.numOfLoans',
                        {
                ...
                <FieldData
                    label={t('pageHeader.loans.fields.totalLoanBalance')}
                    variant={FieldDataVariant.Large}
                >
                ...
                <FieldData
                    ...
                    tooltipBody={t(
                        'pageHeader.loans.fields.eligibleForLoanPopover'
                    )}
                ...
                {loanCarryingBalance ? (
                    <FieldData
                        ...
                        tooltipBody={
                            t(
                                'pageHeader.loans.fields.estimatedNetDeathBenefitPopover'
                            ) as string
                        }
                        tooltipPlacement={PopoverPlacement.TopLeft}
                    >
                    ...
                    <>
                        <FieldData
                            ...
                            tooltipBody={
                                t(
                                    'pageHeader.loans.fields.availableLoanInterestRatePopover'
                                ) as string
                            }
                            tooltipPlacement={PopoverPlacement.TopLeft}
                        >
                        ...
                        <FieldData
                            ...
                            tooltipBody={
                                t(
                                    'pageHeader.loans.fields.availableLoanCreditRatePopover'
                                ) as string
                            }
                            tooltipPlacement={PopoverPlacement.TopLeft}
                        >
            ...
            <div className="mt-8 bg-gray-50 px-8 py-4">
                {newLoanEligibility?.isEligibleNewLoan ? (
                    ...
                    <Tooltip
                        ...
                        body={formatValidationResult(
                            newLoanEligibility?.validationResult
                        )}
                    >
                        <NavElement
                            className="mr-5"

...
export const FormattedEnterprisePhone = ({ phone }: FormattedEnterprisePhoneProps) => {
    return (
...
const BeneficiarySummary = () => {
    ...
    const updatedBenes = useMemo(() => {
        return beneData?.filter((item: any) => item.action === 'UPDATE' || item.action === 'ADD');
    }, [beneData]);
                ...
                return (
                    <div className="my-4 w-full rounded-sm border-2 border-gray-100 p-8" key={item.index}>
                        ...
                        <div className="mb-6">
                            ...
                            <Label className="mt-4 h-6 leading-4.5" label={'Gender'} variant={LabelVariant.FieldLabel} />
                        ...
                        <div className="mb-6">
                            ...
                            <div className="my-4 flex">
                                <div>
                                    <Label variant={LabelVariant.FieldLabel} label={t('allocation.title')} />
                                    <Typography variant={TypographyVariant.BodySm}>
                                        {percentFormatify(allocation, { isInteger: true })}
                                    </Typography>
                                </div>
                                <div className="mx-4">
                                    <Label variant={LabelVariant.FieldLabel} label={t('allocation.relationshipToInsured')} />
                                    <Typography variant={TypographyVariant.BodySm}>
                                        {safeString(item?.party?.allocation?.relationshipToInsured)}
                                    </Typography>
                                </div>
                            </div>
                        ...
                        <div className="mb-5">
                            <Typography variant={TypographyVariant.H2}>{t('contact.title')}</Typography>
                            {item.action === 'UPDATE' && (
                                <div className="my-4 flex">
                                    <div className="mx-2">
                                        <div className="flex">
                                        ...
                                        <div className="flex gap-4">
                                            <div className="bg-gray-50 p-3">
                                                <Tag text={t('current')} className="my-1" />
                                                <FormattedAddress address={currAddress ?? {}} />
                                            </div>
                                            <div className="flex flex-col p-3">
                                                <Tag text={t('updated')} className="my-1" variant={TagVariant.Information} />
                                                <FormattedAddress address={updatedAddress} />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mx-2">
                                        ...
                                        <div className="flex gap-4">
                                            <div className="bg-gray-50 p-3">
                                                <Tag text={t('current')} className="my-1" />
                                                <FormattedEnterprisePhone phone={currentPhones?.[0] ?? {}} />
                                            </div>
                                            <div className="flex flex-col p-3">
                                                <Tag text={t('updated')} className="my-1" variant={TagVariant.Information} />
                                                <FormattedEnterprisePhone phone={updatedPhone} />
                                            </div>
                                        </div>
                                    </div>
                                </div>

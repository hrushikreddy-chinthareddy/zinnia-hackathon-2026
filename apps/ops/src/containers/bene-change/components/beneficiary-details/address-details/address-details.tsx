...
export default function AddressDetails({ setCurrentAddresses, updateAddress, index, isReadOnly, partyType }: AddressDetailsProps) {
    ...
    const addressTypeOptions = [
        ...
        { label: t('addressType.business'), value: ENTERPRISE_ADDRESS_TYPE.BUSINESS },
        { label: t('addressType.secondary'), value: ENTERPRISE_ADDRESS_TYPE.SECONDARY },
    ...
    useEffect(() => {
        ...
        if (address.addressLine1 || address.addressLine2) {
            if (!address.zipCode) {
                setCurrentErrors((prevState: any) => ({ ...prevState, zipCode: t('formValidations.zipCode') }));
            }
            if (!address.state) {
                setCurrentErrors((prevState: any) => ({ ...prevState, state: t('formValidations.state') }));
            }
            if (!address.city) {
                ...
                <div className="mb-3 flex w-full flex-col">
                    ...
                    <div className="my-3">
                        <Field
                            ...
                            onChange={event => {
                                setAddress((prevState: any) => ({ ...prevState, addressLine1: event.target.value }));
                            }}
                            size={FieldSize.Small}

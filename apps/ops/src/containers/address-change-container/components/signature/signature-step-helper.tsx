...
export const useSignatureStepConfig = (t: TFunction, isJointOwnerExist: boolean, isAnnuitant: boolean) => {
    const formValidation = useCallback(
        (signatures: SignatureWithdrawal[]): FormValidationErrors => {
            const errors = {} as FormValidationErrors;

            if (isAnnuitant) {
                const annuitantSignature = signatures?.find(sigInfo => sigInfo?.signType?.text === SignatureValidationTypeWithdrawal.Annuitant);

                // No choice made for signature
                if (annuitantSignature?.isSigned !== false && !annuitantSignature?.isSigned) {
                    errors[`${SignatureValidationTypeWithdrawal.Annuitant}${SignatureFieldNames.SignaturePresent}`] = t('signatureShouldBePresent');
                }
            } else {

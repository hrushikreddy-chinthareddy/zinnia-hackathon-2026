...
export const ApplicableRolesContractSummary = ({
    policy,
}: ApplicableRolesContractSummaryProps) => {
    ...
    const tableData = React.useMemo(
        () => [
            getRolesRecordForCurrentContract(t, policy, applyToRoles),
            getRolesRecordForOtherContract(t, policy, applyToRoles),
        ],
    ...
    return (
        <div className="mb-10 flex w-3/5 flex-col gap-4">
            <Typography variant={TypographyVariant.H2}>
                {t('summary.rolesAndContracts')}
            </Typography>
            {tableData[0].length ? (
                <div>
                    <Typography variant={TypographyVariant.Label}>
                        {t('summary.updatedRoles')}
                    </Typography>
                    <Table
                        bodyCellClass="py-2"
                        columns={getColDefinition(t, [
                            TableColumnFields.PartyRole,
                        ])}
                        data={tableData[0] || []}
                    ></Table>
                </div>

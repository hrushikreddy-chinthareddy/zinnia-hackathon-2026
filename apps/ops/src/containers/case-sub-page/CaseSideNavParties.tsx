...
const NoParties = ({
    ...
    className = '',
    ...rest
}: { caseStatus: string } & HTMLAttributes<HTMLDivElement>) => {
...
const RoleTags = ({ party }: { party: PartyInfo }) => {
    ...
    return (
        <>
            ...
            <div className="whitespace-nowrap self-end">
                {party.roles.length > 1 ? '+ ' + (party.roles.length - 1) : ''}
            </div>
        </>
    );
};
...
const PartyInformation = ({
    party,
    ...rest
}: { party: PartyInfo } & HTMLAttributes<HTMLLIElement>) => {
    if (!party) {
        return null;
    }
    ...
    return (
        ...
        >
            <Icon
                className="flex-shrink min-w-[24px]"
...
export const Parties = ({
    ...
    parties: PartiesProps | undefined;
    caseStatus: string;
    showTitle?: boolean;
    ...
    return (
        <div className="flex w-full flex-col" {...rest}>
            ...
            {noParties && (
                <NoParties caseStatus={caseStatus} className="gap-md" />
            )}
        </div>
    );
};

...
const getTabs = (
    ...
    if (
        ...
        (caseDetails?.caseAdditionalData ?? []).filter(
            (val) => val?.label?.toLowerCase() === 'paymentrecordid'
        )?.length > 0
    ) {
...
export default function CaseSideNavTabs({
    caseDetails,
    sideNavData,
}: {
    caseDetails: Case;
    sideNavData: CaseSideNavProps['data'];
}) {
    const { t } = useTranslation();
    const { featureFlags } = useOptimizely();
    const tabs = getTabs(
        caseDetails,
        featureFlags[FEATURE_FLAGS.DEPU_4253_FUNDING_SOURCES]
    );
            ...
            >
                ...
                >
                    <Parties
                        ...
                        showTitle={tabs.length === 1}
                    />
                </TabContent>
                {showFundingSourceTab && (
                    <TabContent
                        className="w-full"
                        value={TabViews.fundingSource}
                    >
                        <FundingSources
                            fundingSources={
                                caseDetails?.caseAdditionalData?.filter(

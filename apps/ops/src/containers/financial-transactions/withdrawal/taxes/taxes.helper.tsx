...
export const getFormErrors = (federalTaxWithholdings: TaxWithholdingViewModel, stateTaxWithholdings: TaxWithholdingViewModel) => {
    ...
    if (!federalTaxWithholdings.percentAmount && !federalTaxWithholdings.dollarAmount && !federalTaxWithholdings.withholdMinimum && !federalTaxWithholdings.withholdNone) {
    ...
    if (!stateTaxWithholdings.percentAmount && !stateTaxWithholdings.dollarAmount && !stateTaxWithholdings.withholdMinimum && !stateTaxWithholdings.withholdNone) {
...
export const mapTaxWithholdingInstructionsFromViewModel = (viewModel: TaxWithholdingViewModel, withholdingType: TaxWithholdingType): TaxWithholdingInstructions => {
    const taxRateToUse = getTaxRateToUse(viewModel);

    return {
        exemptions: 0,
        // We do not want to set these values if one of the checkboxes is selected
        dollar: taxRateToUse === TaxRateToUse.USEVALUESENTERED && !isNullEmptyOrUndefined(viewModel.dollarAmount)
            ? Number(viewModel.dollarAmount)
            : 0,
        percentage: taxRateToUse === TaxRateToUse.USEVALUESENTERED && !isNullEmptyOrUndefined(viewModel.percentAmount)
...
export const getOwnersTaxJurisdictionState = (policy: Policy): string => {
    ...
    const ownerTaxJurisdiction = policyOwner?.taxWithholdings?.find(tw => tw.taxWithholdingType === TaxWithholdingType.STATE)?.taxJurisdiction;

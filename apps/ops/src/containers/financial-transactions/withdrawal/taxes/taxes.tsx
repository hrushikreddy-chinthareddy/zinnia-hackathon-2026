...
const Taxes = ({ policy }: TaxesProps) => {
    const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'withdrawals.taxes' });

    const { withdrawal, setWithdrawal } = useWithdrawal();
    const { goToNext } = useWorkflow();

    const { taxWithholdingInstructions } = withdrawal;
    const federalWithholdings = taxWithholdingInstructions.find(tw => tw.taxWithholdingType === TaxWithholdingType.FEDERAL);
    const stateWithholdings = taxWithholdingInstructions.find(tw => tw.taxWithholdingType === TaxWithholdingType.STATE);
    const { policyNumber, product } = policy;
    const numberFormat = { type: 'number' as FieldFormat, decimalPlaces: 2, format: '' };

    const [federalTaxWithholdings, setFederalTaxWithholdings] = useState(mapTaxWithholdingInstructionsToViewModel(federalWithholdings));
    const [stateTaxWithholdings, setStateTaxWithholdings] = useState(mapTaxWithholdingInstructionsToViewModel(stateWithholdings));
    ...
    const handleContinue = () => {
        ...
        const withholdingInstuctions = [
            mapTaxWithholdingInstructionsFromViewModel(federalTaxWithholdings, TaxWithholdingType.FEDERAL),
            mapTaxWithholdingInstructionsFromViewModel(stateTaxWithholdings, TaxWithholdingType.STATE),
        ];

        setWithdrawal({ ...withdrawal, taxWithholdingInstructions: withholdingInstuctions });
                        ...
                        <div className="flex flex-col gap-4">
                            <div>
                                <FieldLabel
                                    ...
                                    labelTooltipBody={t('federalTooltip') as string}
                                />
                                        ...
                                        <Field
                                            ...
                                            onChange={e =>
                                                setFederalTaxWithholdings(prevState => ({
                                                    ...prevState,
                                                    percentAmount: xss(e?.target?.value),
                                                }))
                                            }
                                            size={FieldSize.Small}
                                            trailing={<span>%</span>}
                                            type={FieldType.BaseActive}
                                            value={federalTaxWithholdings.percentAmount}
                                            variant={
                                    ...
                                    <div className="flex basis-3/5">
                                        <Field
                                            ...
                                            onChange={e =>
                                                setFederalTaxWithholdings(prevState => ({
                                                    ...prevState,
                                                    dollarAmount: xss(e?.target?.value),
                                                }))
                                            }
                                            size={FieldSize.Small}
                                            type={FieldType.BaseActive}
                                            value={federalTaxWithholdings.dollarAmount}
                                            variant={
                                                federalTaxWithholdings.withholdMinimum || federalTaxWithholdings.withholdNone
                                                    ? FieldVariant.Inactive
                                                    : currentErrors.federalBothInputs
                            ...
                            {currentErrors.federalBothInputs && (
                                <AssistiveText text={t('bothInputsSelected')} variant={AssistiveTextVariant.Error} />
                            )}

                            <CheckboxText
                                checked={federalTaxWithholdings.withholdMinimum}
                                label={t('withholdMiniumum', { type: t('federal') })}
                                onChange={checked => setFederalTaxWithholdings(prevState => ({ ...prevState, withholdMinimum: checked }))}
                            />
                            <CheckboxText
                                checked={federalTaxWithholdings.withholdNone}
                                label={t('dontWithhold', { type: t('federal') })}
                                onChange={checked => setFederalTaxWithholdings(prevState => ({ ...prevState, withholdNone: checked }))}
                            />
                        </div>
                        {currentErrors.federalNothing && (
                            <AssistiveText className="mt-2" text={t('detailsMissing')} variant={AssistiveTextVariant.Error} />
                        )}
                        {currentErrors.federalBothCheckboxes && (
                            <AssistiveText className="mt-2" text={t('bothCheckboxesSelected')} variant={AssistiveTextVariant.Error} />
                        )}
                    </div>
                    <div>
                        <div className="flex flex-col gap-4">
                            <div>
                                <FieldLabel
                                    label={t('stateLabel', { state: ownerTaxState }) as string}
                                    labelTooltip={t('stateLabel', { state: ownerTaxState }) as string}
                                    labelTooltipBody={t('stateTooltip') as string}
                                />
                                <div className="flex gap-4">
                                    <div className="flex basis-2/5">
                                        <Field
                                            ...
                                            trailing={<span>%</span>}
                                            type={FieldType.BaseActive}
                                            value={stateTaxWithholdings.percentAmount}
                                            variant={
                                        ...
                                        <Field
                                            ...
                                            onBlur={event => {
                                                ...
                                                if (!isNullEmptyOrUndefined(value)) {
                                                    ...
                                                    setStateTaxWithholdings(prevState => ({ ...prevState, dollarAmount: formattedValue }));
                                                }
                                            }}
                                            onChange={e =>
                                                setStateTaxWithholdings(prevState => ({
                                            ...
                                            size={FieldSize.Small}
                                            type={FieldType.BaseActive}
                                            value={stateTaxWithholdings.dollarAmount}
                                            variant={
                                                stateTaxWithholdings.withholdMinimum || stateTaxWithholdings.withholdNone
                                                    ? FieldVariant.Inactive
                                                    : currentErrors.stateBothInputs
                            ...
                            <CheckboxText
                                ...
                                label={t('withholdMiniumum', { type: t('state') })}
                                onChange={checked => setStateTaxWithholdings(prevState => ({ ...prevState, withholdMinimum: checked }))}
                            />
                            <CheckboxText
                                checked={stateTaxWithholdings.withholdNone}
                                label={t('dontWithhold', { type: t('state') })}
                                onChange={checked => setStateTaxWithholdings(prevState => ({ ...prevState, withholdNone: checked }))}
                            />
                        </div>

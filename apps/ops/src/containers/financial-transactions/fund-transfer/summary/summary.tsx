...
const Summary = ({ policy, title }: SummaryProps) => {
    const { t } = useTranslation(TranslationFiles.COMMON, {
        keyPrefix: 'fundTransfer.summary',
    });
    ...
    const formatAmount = (value: number | string, type: string) => {
        return type === AmountType.Amount
            ? numberFormatify(value || 0)
            : percentFormatify(value || 0, { isInteger: true });
    };
                    ...
                    <div className="overflow-hidden border border-#CCCCCC rounded-lg">
                        <table className="min-w-full text-sm divide-y divide-[#CCCCCC]">
                            <thead className="bg-[#F8F8F8]">
                                <tr>
                                    <th className="px-4 py-2 text-left w-2/6">
                                        {t('transferedFrom')}
                                    </th>
                                    ...
                                    <th className="px-4 py-2 text-left  border-l border-[#CCCCCC] w-2/6">
                                        {t('transferedTo')}
                                    </th>
                            ...
                            <tbody className="bg-white divide-y divide-[#CCCCCC]">
                                {fundTransfer.funds.transferTo
                                    .filter(
                                        ({ fundName, requestedAmount }) =>
                                            fundName && Number(requestedAmount)
                                    )
                                    .map((to, index) => (
                                        <tr key={index}>
                                ...
                                <tr className="font-semibold">
                                    ...
                                    <td className="px-8 py-2 text-right">
                                        {formatAmount(
                                            fundTransfer.funds.transferTo.reduce(
                                                (sum, to) =>
                                                    sum +
                                                    Number(
                                                        to.requestedAmount || 0

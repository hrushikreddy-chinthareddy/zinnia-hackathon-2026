...
const Transfer = ({
    ...
    const { t } = useTranslation(TranslationFiles.COMMON, {
        keyPrefix: 'workflows.transferStep',
    });
    ...
    const transferToValue = funds.transferTo.reduce(
        (sum, to) => sum + Number(to.requestedAmount || 0),
        0
    );

    const lastTransferToFund = funds.transferTo[funds.transferTo.length - 1];
    const isDisabled =
        lastTransferToFund?.fundId === '' ||
        lastTransferToFund?.requestedAmount === '' ||
        lastTransferToFund?.requestedAmount === 0;
    ...
    const updateTransferFrom = (key: string, fundVal: string) => {
        setFundTransfer((prev) => {
            const updatedTransferFrom = prev.funds.transferFrom.map(
                (item, index) => {
                    ...
                    if (key === FundKey.FundId) {
                        ...
                        const fundName =
                            transferFromFunds?.find(
                                (fund) => fund.value === fundVal
                            )?.label || '';
                        return {
                            ...updatedItem,
                            fundName,
                    ...
                    if (
                        ...
                        Number(fundVal) > Number(availableValue)
                    ) {
                        setErrors((prev) => ({
                            ...prev,
                            ...
                            setErrors((prev) => ({
                                ...
                                transferFromAmount:
                                    transferType === AmountType.Percentage
                                        ? translate('percentageError')
                                        : translate('amountError'),
                            }));
                        }
                    }
                    return updatedItem;
        ...
        setFundTransfer((prev) => {
            const updatedTransferTo = prev.funds.transferTo.map((item, i) => {
                ...
                if (key === FundKey.FundId) {
                    ...
                    const fundName =
                        transferToFunds?.find((fund) => fund.value === fundVal)
                            ?.label || '';
                    return {
                        ...updatedItem,
                        fundName,
    ...
    const secondaryCta = {
        ...
        onClick: () => {
            router.push(
                `/policies/${product?.planCode}/${policyNumber}/policy/${ParentPage.Funds}`
            );
        },
    };
    ...
    const noAvailableDestinationFunds =
        ...
        (transferToFunds.length === 1 &&
            transferToFunds[0]?.value === transferFromFunds[0]?.value);
        ...
        <>
            ...
            >
                ...
                <SelectSimple
                    ...
                    onChange={(value) =>
                        handleTransferType(value as AmountType)
                    }
                    size={FieldSize.Small}
                    ...
                    <div className="grid sm:grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="grid grid-cols-[2fr_1fr] gap-4">
                                <div className="w-full min-w-[100px] max-w-full">
                                    <SelectSimple
                                        ...
                                        value={
                                            fundTransfer?.funds?.transferFrom[0]
                                                ?.fundId
                                        }
                                        variant={
                                            errors.transferFromFund
                                                ? FieldVariant.Error
                                                : FieldVariant.Default
                                        }
                        ...
                        <div className="bg-gray-50 p-8 rounded-lg">
                            ...
                            <div className="grid grid-cols-[7fr_4fr] gap-4 mb-2">
                                {fundTransfer?.funds?.transferTo.map(
                                    (fund, index) => {
                                        const filteredTransferToFunds =
                                            transferToFunds.filter(
                                                (option) =>
                                                    !funds.transferTo.some(
                                                        (f, i) =>
                                                            f.fundId ===
                                                                option.value &&
                                                            i !== index
                                        ...
                                        const disableCondition =
                                            !fetchingFunds &&
                                            (noAvailableDestinationFunds ||
                                                filteredTransferToFunds.length ===
                                                    0);
                                            ...
                                            <>
                                                <div className="w-full min-w-[100px] max-w-full">
                                                    <SelectSimple
                                                        ...
                                                        label=""
                                                        options={
                                                            filteredTransferToFunds
                                                        }
                                                        onChange={(e) =>
                                                            updateTransferToFund(
                                                                index,
                                                                FundKey.FundId,
                                                                e
                                                            )
                                                    ...
                                                    <div className="flex items-start gap-4 ">
                                                        <div>
                                                            <Field
                                                                ...
                                                                value={
                                                                    fund?.requestedAmount as string
                                                                }
                                                                onChange={(
                                                                    e
                                                                ) => {

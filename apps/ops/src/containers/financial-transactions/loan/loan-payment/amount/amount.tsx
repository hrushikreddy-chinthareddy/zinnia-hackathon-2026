...
const Amount = ({ policy }: AmountProps) => {
    ...
    const validateFields = (effectiveDate: string, paymentAmount: string) => {
        ...
        if (
            isNullEmptyOrUndefined(effectiveDate) ||
            !dayjs(effectiveDate, NUMERIC_DATE_FORMAT).isValid()
        ) {
        ...
        if (
            ...
            errors = {
                ...errors,
                paymentAmount: `${t('invalidPaymentAmountError')}`,
            };
        }
    ...
    const handleContinue = () => {
        if (
            !validateFields(
                loanPayment.effectiveDate,
                String(loanPayment.paymentAmount)
            )
        ) {
            return;
        }

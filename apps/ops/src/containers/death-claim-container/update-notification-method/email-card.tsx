...
const EmailCard = ({ email, setEmail }: EmailCardProps) => {
  ...
  const handleChangeEmail = () => {
    sidesheet.changeSideSheetContent(t('updateEmail'), <UpdateEmailAddress email={email} setEmail={setEmail} handleClose={handleClose}/>)
  ...
  return (
    <div className="p-4 border-1 rounded-md border-gray-200 flex justify-between gap-6 items-center w-full">
      ...
      <div>
        <Button size={ButtonSize.Small} mode="link" onClick={handleChangeEmail}>
          {t('change')}
        </Button>
      </div>
    </div>
...
const UpdateEmailAddress = ({ email: initialEmail, setEmail, isContainerClass = true, isCancel = true, handleClose }: UpdateEmailAddressProps) => {
  const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'updateNotificationMethodForBeneficiary.updateNotificationMethodStep.notificationMethods' });
    ...
    <CardContainer classNames={'w-full'} containerClassNames={isContainerClass ? 'w-full content-divider' : 'w-full'}>
      <div className='mt-4'>
        ...
        <div className="mt-6 flex ">
          <Button className="mr-4" onClick={handleContinue} size={ButtonSize.Small}>

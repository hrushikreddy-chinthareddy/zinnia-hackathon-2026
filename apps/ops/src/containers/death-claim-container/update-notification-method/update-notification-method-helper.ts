...
import { AddressNotificationMethod, EmailNotificationMethod, FaxNotificationMethod, NotificationsTransactionData } from "@deps/components/side-sheet/side-sheet-case-step-details/tabs/bene-notification-tab.types";
...
export const buildUpdateNotificationMethodPayload = (
  ...
  const zlCaseId = identifiers?.find(item => item.identifier === 'zlCaseId')?.value;
  const payload = {
    correlationId,
    zlCaseId,
    sorSystem: 'LifeCad',
    ...
    beneficiaryChangeDetail: {
      ...
      notificationPreferences: {
        notificationMethod: {
          ...
          action: entity?.notificationPreferences?.notificationMethod?.method !== updatedNotificationMethod ? ClaimActionTypes.UPDATE : ClaimActionTypes.NONE
        },
        fax: {
        ...
        address: {
          ...addressData, action: addressData?.action || ClaimActionTypes.NONE
        }
      }
    }
  };

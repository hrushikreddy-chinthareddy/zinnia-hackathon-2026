...
const FaxCard = ({ fax, setFax }: FaxCardProps) => {
  ...
  const handleChangeFax = () => {
    sidesheet.changeSideSheetContent(t('updateFax'), <UpdateFaxNumber fax={fax} setFax={setFax} handleClose={handleClose} />);
  ...
  return (
    <div className="p-4 border-1 rounded-md border-gray-200 flex justify-between gap-6 items-center w-full">
      ...
      <div>
        <Button size={ButtonSize.Small} mode="link" onClick={handleChangeFax}>
          {t('change')}
        </Button>
      </div>
    </div>
...
const UpdateFaxNumber = ({ fax: initialFax, setFax, isContainerClass = true, isCancel = true, handleClose }: UpdateFaxProps) => {
  const { t } = useTranslation(TranslationFiles.COMMON, { keyPrefix: 'updateNotificationMethodForBeneficiary.updateNotificationMethodStep.notificationMethods' });

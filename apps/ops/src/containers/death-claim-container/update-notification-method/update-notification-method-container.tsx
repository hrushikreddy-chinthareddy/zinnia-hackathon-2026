...
const UpdateNotificationMethodContainer = ({ transactionData, policy }: UpdateNotificationMethodContainerProps) => {
  ...
  const steps = useMemo(() => [
      {
        ...
        component: <UpdateNotificationMethodStep policy={policy} transactionData={transactionData} />,
        index: 0,
      ...
      },
      {
        ...
        isVisible: () => true,
        component: (
          <ConfirmStep policy={policy} transactionData={transactionData} />
        ),
  ...
  const filteredSteps: Step[] = useMemo(() => steps.filter((item: any) => item.isVisible?.()).map((item: any, index: number) => ({ ...item, index })),
    [steps]);

...
export const PartyCard = ({
    ...
    const [isSelected, setIsSelcted] = useState<number | null>(selectedItem?.party?.partyRoleId  || null);
            ...
            {parties?.map((item, index) => (
                <ClickContainer
                    classes={clsx(
                        'flex w-min py-4',
                        {
                            'border-primary hover:border-primary ': item.party.partyRoleId === isSelected,
                        },
                        'min-h-[80px] min-w-[300px]'
                    )}
                    ariaLabel={item.party?.partyRole ?? ''}
                    onClick={() => handleClick(item?.party?.partyRoleId ?? null, item)}
                    key={`container-${index}`}
                    isSelected={item.party.partyRoleId === isSelected}
                >
                    <div key={index}>
                        <div className="flex flex-wrap gap-1">
                            <Tag key={item.party?.partyRole ?? '' + index} text={convertToChipText(item.party?.partyRole, t) ?? ''} />
                        </div>
                        <div>
                            <Typography variant={TypographyVariant.BodySm} className="py-2">
                                <PiiWrapper>
                                    {toTitleCase([item.party.firstName, item.party.lastName].filter(Boolean).join(' '))}
                                </PiiWrapper>
                            </Typography>
                        </div>
                    </div>

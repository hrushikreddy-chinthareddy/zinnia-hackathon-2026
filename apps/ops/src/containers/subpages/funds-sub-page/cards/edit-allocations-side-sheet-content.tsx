...
export const EditAllocationsContent: React.FC<IEditAllocationsContent> = ({
    ...
    if (success) {
        return (
            <EditAllocationSuccess caseId={successCaseId} sideSheet={sideSheet} policyOwner={policyOwner} />
        )
    }

    if (isSystenDown) {
        return (
            <EditAllocationsSystemDown sideSheet={sideSheet} setIsSystenDown={setIsSystenDown} setIsNigo={setIsNigo} setIsSuccessFlow={setIsSuccessFlow} />
        )
    }
    const getAllocationPayload = () => {
        const effectiveDateFormatted = effectiveDate.length > 0 ? dayjs(effectiveDate, NUMERIC_DATE_FORMAT).format(ZAHARA_API_DATE_FORMAT) : '';
    ...
    if (isSuccessFlow) {
        return (
            <AllocationSuccessFlow allFunds={allFunds} total={total} sideSheet={sideSheet} submitHandler={submitHandler} setIsSuccessFlow={setIsSuccessFlow} />
        )
    }
    ...
    if (isNigo) {
        return (
            <EditAllocationNigo
                allFunds={allFunds} setIsNigo={setIsNigo} total={total} error={error}
                sideSheet={sideSheet} submitHandler={submitHandler} />
        )
    }

    if (nigoSuccess) {
        return (
            <EditAllocationsNigoSuccess sideSheet={sideSheet} />
        )
    }
    ...
    return (
        <div className="flex flex-col p-8 h-full">
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                        {selectedCaseId === PROCESS_WITHOUT_CASE_DOCUMENT && (
                            <AssistiveText
                                ...
                                text={true ? t('workflows.start.processWithoutDocAssistiveTextWithOnBaseUpdate') : t('workflows.start.processWithoutDocAssistiveText')}
                            />
                        )}
                        {showSelectionError && (
            ...
            <div className="pt-1">
                {effectiveDateError && (
                    <AssistiveText variant={AssistiveTextVariant.Error} text={t('fundAllocation.effectiveDateError')} />
                )}
            </div>
            ...
            <div className="flex justify-end">
                {error && <div className="flex items-center pt-2"><AssistiveText variant={AssistiveTextVariant.Error} text={t(error)} /></div>}
            </div>

            <div className="flex pt-10">
                <span className="mr-6">
                    <SpinnerButton stopLoading={stopLoading} size={ButtonSize.Small} text={t('fundAllocation.updateAllocations')} onClick={updateAllocationHandler} />
                </span>
                <NavElement
            ...
            {!stopLoading && <div className="pt-4">
                <AssistiveText

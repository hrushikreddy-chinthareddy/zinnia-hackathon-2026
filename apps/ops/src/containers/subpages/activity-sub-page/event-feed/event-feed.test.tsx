...
import {
    ...
    pendingSubsequentTransaction,
} from './event-feed-test.helpers';
...
describe.skip('EventFeed', () => {
    it('should render correctly when empty', async () => {
        ...
        render(
            <PolicyData.Provider
                value={{
                    policy: basePolicy,
                    policyDetails: new PolicyDetails(basePolicy),
                    refreshPolicy: () => null,
                }}
            >
        ...
        await waitFor(() => {
            expect(
                screen.getByText('policy.history.completedEvents')
            ).toBeInTheDocument();
            expect(
                screen.getByText('policy.history.noEventsTitle')
            ).toBeInTheDocument();
            expect(
                screen.getByText('policy.history.noEventsSubtitle')
            ).toBeInTheDocument();
            expect(
                screen.queryByText('policy.history.upcomingEvents')
    ...
    it('should render correctly when there are only upcoming events', async () => {
        ...
        mockedGetTransactions.mockReturnValueOnce(
            Promise.resolve([pendingSubsequentTransaction])
        );
        ...
        await waitFor(() => {
            ...
            expect(
                screen.getByText('policy.history.upcomingEvents')
            ).toBeInTheDocument();
            expect(
                screen.getByText('historyEventCard.bankingBody')
            ).toBeInTheDocument();
            expect(
                screen.getByText('historyEventCard.premiumAutopay')
            ).toBeInTheDocument();
    ...
    it('should render correctly when there are only completed events including non-financial', async () => {
        mockedGetTransactions.mockReturnValueOnce(
            Promise.resolve([newLoanNonTransaction, completedSubsequent])
        );
        ...
        await waitFor(() => {
            expect(
                screen.queryByText('policy.history.noEventsTitle')
            ).not.toBeInTheDocument();
            expect(
                screen.queryByText('policy.history.noEventsSubtitle')
            ).not.toBeInTheDocument();
            expect(
                screen.queryByText('policy.history.upcomingEvents')
            ).not.toBeInTheDocument();
            ...
            expect(
                screen.getByText('historyEventCard.loan')
            ).toBeInTheDocument();
    ...
    it('should render correctly with uncompleted transactions and completed spanning years', async () => {
        ...
        await waitFor(() => {
            ...
            expect(
                screen.queryAllByText('historyEventCard.premiumAutopay')
            ).toHaveLength(3);
            expect(
                screen.queryAllByText('historyEventCard.bankingBody')
            ).toHaveLength(3);

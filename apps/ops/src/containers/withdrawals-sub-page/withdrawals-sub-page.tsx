...
const WithdrawalsSubPage = ({ policy }: WithdrawalsSubPageProps) => {
    ...
    const withdrawalEnabled = featureFlags[FEATURE_FLAGS.SYSTEMATIC_WITHDRAWAL_TRANSACTION];
    ...
    const { data: withdrawalEligibility } = useQuery({
        queryKey: ['checkEligibilityWithdrawal', planCode, policyNumber, withdrawalProgram?.arrangementId],
        queryFn:
            withdrawalEnabled && planCode && policyNumber
                ? () =>
                      checkSystematicProgramEligibilityQuery(planCode, policyNumber, withdrawalProgram?.arrangementId ?? '', {
                          systematicProgram: { arrangementType: ArrangementType.WITHDRAWAL },
                      })
                : skipToken,
        select: data => ({
            isEligibleWithdrawal: data?.status === TransactionResponseStatus.Success,
            ineligibleWithdrawalReason: formatValidationResult(data?.validationResult),
        }),
    });

    const { data: rmdEligibility } = useQuery({
        queryKey: ['checkEligibilityRmd', planCode, policyNumber, rmdProgram?.arrangementId],
        queryFn:
            planCode && policyNumber && rmdEnabled
                ? () =>
                      checkSystematicProgramEligibilityQuery(planCode, policyNumber, rmdProgram?.arrangementId ?? '', {
                          systematicProgram: { arrangementType: ArrangementType.REQUIREDMINIMUMDISTRIBUTION },
                      })
                : skipToken,
        select: data => ({
    ...
    const openCancelSideSheet = (type: string) => {
        ...
        const arrangementType =
            type === ArrangementType.REQUIREDMINIMUMDISTRIBUTION ? ArrangementType.REQUIREDMINIMUMDISTRIBUTION : ArrangementType.WITHDRAWAL;
        const systematicProgramReason =
            type === ArrangementType.REQUIREDMINIMUMDISTRIBUTION ? Reason.REQUIREDMINIMUMDISTRIBUTION : Reason.WITHDRAWAL;

        console.log('getPayout(arrangementType)?.nextProgramDate', getPayout(arrangementType)?.nextProgramDate);
        sideSheet.changeSideSheetContent(
            <Typography variant={TypographyVariant.H2}>{sideSheetTitle}</Typography>,

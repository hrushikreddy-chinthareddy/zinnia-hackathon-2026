...
const TabGroupContent = ({
    ...
    const fetchCases = useCallback(async () => {
        try {
            ...
            if ('total' in response) {
                ...
                const caseNumber = response.data.filter(
                    (item) => new Date(item.updatedAt) >= ninetyDaysAgo
                ).length;
                ...
                setError([
                    ...
                    {
                        text: `${caseNumber} ${t(
                            'sideSheet.caseDetailsContent.otherOpenCases'
                        )} `,
                        color: 'rgb(var(--color-secondary))',
                    },
                    {
                        text: t(
                            'sideSheet.caseDetailsContent.relatedToThisPolicy'
                        ),
                    },
                ]);
            } else {
    ...
    const { marketingName, planCode, productType, status, tooltip, variant } =
        globalValuesData;
    ...
    const openSideSheet = () => {
        ...
        sideSheet.changeSideSheetContent(
            t('nigoEntry.documentPanel.documents'),
            content
        );

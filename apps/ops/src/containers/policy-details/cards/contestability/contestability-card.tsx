...
const ContestabilityCard: React.FC<ContestabilityCardProps> = ({
    policyDetails,
}) => {
    const { t } = useTranslation();
    const endDate = convertKebabedDateString(
        policyDetails.contestabilityEndDate
    );
    const startDate = convertKebabedDateString(
        policyDetails.contestabilityStartDate
    );
    const inactive =
        dayjs() >=
        dayjs(endDate, [
    ...
    const tooltip = inactive
        ? t(`${BASE_KEY}inactiveBadgeTooltip`)
        : t(`${BASE_KEY}activeBadgeTooltip`);
    const label = inactive
        ? t(`${BASE_KEY}inactiveBadgeLabel`)
        : t(`${BASE_KEY}activeBadgeLabel`);
    ...
    return (
        <CardContainer containerClassNames="border-b-2 border-gray-100">
            ...
            <ContestabilityContent
                contestabilityStartDate={startDate}
                contestabilityEndDate={endDate}
            />
        </CardContainer>
    );
};

...
const AdditionalInformationCard = ({
    ...
    const netAmountRiskValue = numberFormatify(
        policyDetails?.netAmountAtRisk,
        currencyFormat
    );
    ...
    return (
        <CardContainer containerClassNames="rounded-b">
            <Typography variant={TypographyVariant.H2}>
                {t(`${BASE_KEY}.additionalInformation`)}
            </Typography>
            <div className="mt-4 flex flex-col gap-8 sm:flex-row">
                {!policyDetails.isAnnuity && (
                    <div>
                        <div className="flex items-center gap-2">
                            <Label
                                label={t(`${BASE_KEY}.netAmountRisk`)}
                ...
                <div>
                    ...
                    <Content
                        details={
                            policyDetails?.planCode ?? DEFAULT_ERROR_STRING
                        }
                        variant={ContentVariant.BodySm}
                    />
                </div>

...
export const OtpBuildFormProcess = (
...
export const WithdrawalBuildUpdate = (
    status: CaseStatus | TaskStatus,
    formState: OtpWithdrawalFormState
): OtpWithdrawalFormState => {
    const amountDetails = { ...formState.formProgram };
    if (
        amountDetails?.programType?.text === ProgramType.Withdrawal &&
        amountDetails?.partialAmount?.text
    ) {
        amountDetails.programSubType = {
            text: formState.fundWithdrawnMethod as FundWithdrawnMethod,
        };
    }
    ...
    if (status === CaseStatus.Submit || status === TaskStatus.Completed) {
        if (
            formState.fundWithdrawnMethod === FundWithdrawnMethod.SpecifyFunds
        ) {
            distributionInstruction.funds =
                distributionInstruction.funds.filter(
    ...
    return {
        ...formState,
        formDisbursement: updatedDisbursement,
        formDistribution: distributionInstruction,
        formProgram: amountDetails,
    };
};

export const RMDBuildUpdate = (
    status: CaseStatus | TaskStatus,
    formState: OtpWithdrawalFormState
): OtpWithdrawalFormState => {
    const amountDetails = { ...formState.formProgram };
    const distributionInstruction = { ...formState.formDistribution };
    const funds = distributionInstruction.funds.filter(
        (fund) => !!fund.amount.text
    );
    ...
    if (
        ...
        [TaskStatus.New, TaskStatus.InProgress, TaskStatus.Completed].includes(
        ...
        if (
            ...
            const programSubType =
                funds?.[0]?.amount?.amountType === AmountType.Percent
                    ? ProgramSubType.Percentage
                    : ProgramSubType.Dollar; // CMW-13427

            amountDetails.programSubType = {
                text:
                    funds.length > 0 ? programSubType : ProgramSubType.Prorata,
            }; // CMW-13800 set programSubType default prorata in case of selected specify funds & funds not added
        } else {
            distributionInstruction.funds = [];

...
const StatusesForSaveAsDraftAction: string[] = [
    CaseStatus.Pending,
    TaskStatus.New,
...
export const FormProvider = ({
    ...
    const [formDisbursement, setFormDisbursement] = useState(
        form.data.formRequest.formDisbursement
    );
    const [formDistribution, setFormDistribution] = useState(
        form.data.formRequest.formDistribution
    );
    const [formFullSurrenderAck, setFormFullSurrenderAck] = useState(
        form.data.formRequest.formFullSurrenderAck
    );
    ...
    const [formOL4753Data, setFormOL4753Data] = useState(
        form.data.formRequest.formOL4753Data || null
    );
    ...
    const [formProgram, setFormProgram] = useState(
        form.data.formRequest.formProgram
    );
    const [formRestriction, setFormRestriction] = useState(
        form.data.formRequest.formRestriction
    );
    const [formSignature, setFormSignature] = useState(
        form.data.formRequest.formSignature
    );
    const [formSource, setFormSource] = useState(
        form.data.formRequest.formSource
    );
    const [formTaxWithholding, setFormTaxWithholding] = useState(
        form.data.formRequest.formTaxWithholding
    );
    const [formTpaAuthorization, setFormTpaAuthorization] = useState(
        form.data.formRequest.formTpaAuthorization
    );
    const [formSurrenderingCompany, setFormSurrenderingCompany] = useState(
        form.data.formRequest.formSurrenderingCompany || null
    );
    const [formAdditionalWaivers, setFormAdditionalWaivers] = useState(
        form.data.formRequest.formAdditionalWaivers || null
    );
    const [formNigos, setFormNigos] = useState(
        form.data.formRequest.formNigos || null
    );
    const [formReindexingData, setFormReindexingData] = useState(
        form.data.formRequest.formReindexingData || null
    );
    const [currentFormState, setCurrentFormState] = useState(form.status);
    const [formComment, setFormComment] = useState(
        form?.data?.formRequest?.formComment || ({} as FormComment)
    );
    ...
    const [ownerAcknowledgement, setOwnerAcknowledgement] = useState(
        form.data.formRequest.ownerAcknowledgement
    );
    // Derived states from form data
    const [fundWithdrawnMethod, setFundWithdrawnMethod] = useState(
        getFundWithdrawnMethod(form) || null
    );
    const [ownerStateOfResidence, setOwnerStateOfResidence] = useState(
        getOwnerStateOfResidence(form.data.formRequest.formParty)
    );
    const [contractIssueState, setContractIssueState] = useState(issueState);

    // Meta info
    const selectedFormSubType = formData?.formExtName?.includes(
        FormSubtype.FullWithdrawal.toUpperCase()
    )
    ...
    const [formValidator, setFormValidator] = useState<
        (val?: FormParts) => FormValidationErrors
    >(() => () => {
        return {};
    });
    const [formBeneInfo, setFormBeneInfo] = useState(
        form.data.formRequest?.formBeneInfo || null
    );
    const [formPeriodicPension, setFormPeriodicPension] = useState(
        form.data.formRequest?.periodicPensionForm || null
    );
    const [formESignatureData, setFormESignatureData] = useState(
        getDefaultESignatureData(
            form.data.formRequest?.formESignatureData ||
                ({} as FormEsignatureData),
            formParty
        )
    );
        ...
        if (Array.isArray(formIrsData)) {
            const updatedFormIrsData = formIrsData.map((data) => {
                if (!Array.isArray(data.irsTaxWithholding)) {
                    return {
                        ...data,
                        irsTaxWithholding: data.irsTaxWithholding
                            ? [data.irsTaxWithholding]
                            : [],
                    };
                }
                return data;

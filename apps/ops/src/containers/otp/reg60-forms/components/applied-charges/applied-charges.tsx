...
const AppliedCharges = ({
    ...
    const [applyCharges, setApplyCharges] = useState<ApplyChargesType>({ applicable: isChecked, amount: amountValue });

    const handleCheckBox = () => {
        if (applyCharges.applicable) {
            onDataChange({ applicable: !applyCharges.applicable, amount: '' });
            setApplyCharges({ applicable: !applyCharges.applicable, amount: '' });
        } else {
            onDataChange({ ...applyCharges, applicable: !applyCharges.applicable });
            setApplyCharges(oldState => ({ ...oldState, applicable: !applyCharges.applicable }));
        }
    };
    ...
    return (
        <div className={applyCharges.applicable ? ' rounded-md border-2 border-gray-200 p-3' : 'my-6'}>
            <CheckboxText
            ...
            {applyCharges.applicable ? (
                <div className="my-2 mt-4">
                    <Field
                        ...
                        value={isNaN(applyCharges.amount as number) ? '' : (applyCharges.amount as string)}
                        message={formError}

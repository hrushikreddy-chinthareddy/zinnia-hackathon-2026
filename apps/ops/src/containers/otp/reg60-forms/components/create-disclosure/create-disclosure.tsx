...
const CreateDisclosure = ({
    ...
    const isComparisonLimitReached =
        allContractComparison?.length >= MAX_COMPARISON;

    const handleRemoveComparison = (idx: number) => {
        const result = [
            ...allContractComparison.slice(0, idx),
            ...allContractComparison.slice(idx + 1),
        ];
    ...
    const handleAddNewComparison = () => {
        ...
        const newComparison = [
            ...allContractComparison,
            {
                ...
                comparisonId:
                    allContractComparison[allContractComparison.length - 1]
                        .comparisonId + 1,
            },
        ];
        onDisclosureChange({
            ...disclosure,
            contractComparison: newComparison,
        });
    };

    const handleComparisonContractChange = (
        comparisonData: ContractComparison,
        changedIndex: number
    ) => {
        const updatedComp = allContractComparison?.map(
            (contractComp: ContractComparison, ind: number) => {
                if (changedIndex !== ind) {
                    return contractComp;
                }
    ...
    const handelAnnuityUpdate = (annuityData: AnnuityQuote) => {
        onDisclosureChange({
            ...disclosure,
            proposedAnnuitizationQuote: annuityData,
        });
    };

    const renderProposedAnnuity = (disclosureAuthorization.product ===
        Products.retireEase ||
    ...
    return (
        <div>
            <div
                role="button"
                className="float-right my-[-40px] px-4 py-[5px]"
                onClick={() => setCurrentPage(CurrentPage.INFO)}
            >
                <Breadcrumb
                    h1={''}
                    text={t('contractComparison.back') || 'back'}
                    url={'#'}
                />
            </div>
            <Typography
                ...
                data-testid="disclosure-title"
            >
                {t('contractComparison.createDisclosureHeader')}
            </Typography>
            {renderProposedAnnuity}
            {allContractComparison &&
                allContractComparison?.map(
                    (contract: ContractComparison, index: number) => {
                        return (
                            <div
                                key={
                                    'contract-comparison' +
                                    contract?.comparisonId
                                }
                                data-testid="comparison-contract"
                            >
                                <div
                                    className={
                                        index >= 1
                                            ? 'my-4 border-t-2 pb-4 '
                                            : 'my-4 pb-4'
                                    }
                                >
                                    {index >= 1 && (
                                    ...
                                    <ComparisonContract
                                        ...
                                        onComparisonContractChange={(
                                            val: ContractComparison
                                        ) =>
                                            handleComparisonContractChange(
                                                val,
                                                index
                                            )
                                        }
                                        formErrors={
                                            formErrors[
                                                index
                                            ] as unknown as FormValidationErrors
                                        }
                                    />
                                </div>
                            </div>
            ...
            <div className=" self-center p-4">
                {allContractComparison.length <= MAX_COMPARISON && (
                    <Button
                        disabled={
                            isComparisonLimitReached || isFormStateReadOnly
                        }
                        variant={
                            isComparisonLimitReached || isFormStateReadOnly

...
function BeneficiaryNotificationChange({ beneficiary, setBeneficiary, task, setAddressSelected, t }: BeneficiaryNotificationChangeProps) {
  ...
  const handleAddressSubmit = (addressData: any) => {
    ...
    const updatedAddress = { ...beneficiary.notificationPreferences.address, ...addressData, action: ClaimActionTypes.UPDATE };
  ...
  const renderNotificationComponent = () => {
    switch (beneficiary.notificationPreferences.notificationMethod.method) {
      ...
      case ClaimCommunicationTypes.Email:
        return (
          <div className="col-span-4 mt-4">
            <EmailAddress email={beneficiary.notificationPreferences.email?.emailAddress ?? ''} setEmail={(val: string) => setBeneficiary({
              ...beneficiary,
      ...
      case ClaimCommunicationTypes.Fax:
        return (
          <div className="col-span-4 mt-4">
            <FaxNumber fax={beneficiary.notificationPreferences.fax?.faxNumber ?? ''} setFax={(val: string) => setBeneficiary({
              ...beneficiary, notificationPreferences: {
                ...beneficiary.notificationPreferences,
  ...
  return (
    <>
      <ButtonGrp
        activeValue={beneficiary.notificationPreferences.notificationMethod.method || ''}
        ...
        toggle={value => {
          setBeneficiary({
            ...
            notificationPreferences: {
              ...
              notificationMethod: { action: ClaimActionTypes.UPDATE, method: value }
            }
          })
        }}
        size="lg"
        labels={[
          { label: t('paperMail'), value: ClaimCommunicationTypes.Mail },

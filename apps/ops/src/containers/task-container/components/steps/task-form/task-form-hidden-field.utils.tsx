...
export const applyHiddenFieldPopulation = (formData: any, uiSchema: any): any => {
...
const extractHiddenFieldMappings = (uiNode: any, path: string[] = []): HiddenFieldMapping[] => {
  ...
  for (const key in uiNode) {
    ...
    if (value?.['ui:options']?.hiddenFieldToPopulate) {
      ...
      const targetPath = parentPath ? parentPath + '.' + targetFieldName : targetFieldName;
      mappings.push({ sourcePath, targetPath });
    }
    // Recurse if nested object
    if (typeof value === 'object' && value !== null) {
      mappings = mappings.concat(extractHiddenFieldMappings(value, currentPath));
    }
  }
  return mappings;
}

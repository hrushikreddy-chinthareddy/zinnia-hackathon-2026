...
export const NigoDetails = ({
    ...
    const { exceptions, setExceptions, messages, setMessages } = useTask();
                ...
                <div className="flex-1">
                    {nigoExceptions?.map((item: NigoException, index: Key) => {
                        const subExceptions = nigoSubExceptions?.find(
                            (subItem: NigoSubException) =>
                                subItem.nmId === item.value
                        )?.subExceptions;
                        return (
                            <div key={`{exception-${index}}`} className="mt-2">
                                <CheckboxText
                                    ...
                                    onChange={(isChecked: boolean) =>
                                        onExceptionChange(item.value, isChecked)
                                    }
                                />

    ...
    const dynamicSteps = taskMetadata.map((metadata, index) => ({
        ...
        component: (
            <div className="bg-white shadow-elevation-light-04">
                <TaskFormStep taskInfoLink={taskInfoLink} isSubmit={true} taskMetadata={metadata} key={`step_${index}`}></TaskFormStep>
            </div>
        ),
        text: metadata?.title || '',
        isSubmit: true,
    ...
    const staticSteps: Step[] = [
        {
            ...
            component: (
                <div className="bg-white shadow-elevation-light-04 mb-5 ">
                    {' '}
                    <ConfirmStep taskType={taskType} taskInfoLink={taskInfoLink} isCta={true} ctaLink={`/cases/${task?.caseId}/progress`} />
                </div>
            ),

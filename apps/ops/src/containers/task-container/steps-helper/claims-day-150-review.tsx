...
export const getDay150ReviewSteps = ({ taskType, taskInfoLink, t, task, taskMetadata }: GetStepsProps) => {
  ...
  const steps: Step[] = [
    {
      isVisible: () => true,
      component: <ClaimBeneStatus beneficiary={beneficiary} setBeneficiary={setBeneficiary} />,
    ...
    },
    {
      isVisible: () => true,
      component: (
        <MemoizedTaskFormStep taskInfoLink={taskInfoLink} isSubmit={task.data.details.benefinalcontactattempt?.beneficiaryChangeDetail?.changeType === "BENEFICIARY_ADDRESS_CHANGE"}
          taskMetadata={taskMetadata[0]} key={`step_${0}`} stepIndex={task.data.details.benefinalcontactattempt?.beneficiaryChangeDetail?.changeType === "BENEFICIARY_ADDRESS_CHANGE" ? 3 : 2}></MemoizedTaskFormStep>
      ),
    ...
    },
    {
      isVisible: () => true,
      component: (
        <Claims150Call beneficiary={beneficiary} setBeneficiary={setBeneficiary} taskType={taskType} />
      ),
      text: t('tabs.call'),
      index: 2,

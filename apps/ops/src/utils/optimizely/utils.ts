...
export const isFastFeatureEnabled = (taskType: string, featureFlagMap: FeatureFlags | undefined): boolean => {
    if(!featureFlagMap) {
        return false;
    }
    const type = TaskToProcessType[taskType.toUpperCase() as keyof typeof TaskToProcessType];

...
export const getEnvironment = () => {
  ...
  let environment = ENVIRONMENT_NAME.DEV;
  if (!url?.length || url in ENVIRONMENT_URLS) return ENVIRONMENT_NAME.DEV;
  if (url in ENVIRONMENT_URLS) {
    const [environmentName] = Object.entries(ENVIRONMENT_URLS).find(([_key, value]) => {
      void (_key);
      return value === url
    }) || [];

    environment = environmentName as ENVIRONMENT_NAME;

...
function encryptData(plainText: string, base64Key: string): { encrypted: string; iv: string } {
...
export function decryptData(encryptedText: string, ivBase64: string, base64Key: string): string {
    const key = Uint8Array.from(Buffer.from(base64Key, 'base64')); // Decode Base64 key

    ...
    const filters = useMemo(() => {
        return Object.keys(queryParams).reduce((acc, qpKey) => {
            if (queryKeys.includes(qpKey)) {
                try {
                    acc[qpKey] = JSON.parse(
                        xss(
                            JSON.stringify(
                                queryParams[qpKey as keyof typeof queryParams]
                            )
                        )
                    );
                } catch (error) {
                    console.error(
                        'queryStoreFilters::Error parsing query param',
                        error
                    );
                }
            }
        ...
        useCallback(
            (newFilters: SetStateAction<ParsedUrlQueryInput>) => {
                ...
                setQueryParams((prevQueryParams) => {
                    const paramsWithoutFilters = Object.keys(
                        prevQueryParams
                    ).reduce((acc, qpKey) => {
                        if (!queryKeys.includes(qpKey)) {
                            acc[qpKey] =
                                prevQueryParams[
                                    qpKey as keyof typeof prevQueryParams
                                ];
                        }
                        return acc;
                    }, {} as ParsedUrlQueryInput);

...
export const getTaskFormMetadataSSRMock = async (clientId: string, taskType: TaskType): Promise<FormMetadata | null> => {
    ...
    try {
        ...
        const schema = await import(`@deps/jsonschema-mock-service/tasks/${carrier}/${schemaFileName}.json`);
        return schema.default as unknown as FormMetadata;
    } catch (error) {
        console.warn(`No schema found for task type: ${taskType}. Returning default schema.`, error);
        return null; // Or return a default schema if applicable
    }
};

export const getCaseTaskByIdSSRMock = async (taskType: TaskType, clientId?: string): Promise<ManagementTask<TaskStatus> | null> => {
    let carrier: string = DEFAULT_CARRIER;
    if (clientId) {
        carrier = clientId;
    }
    try {
        ...
        const taskData = await import(`@deps/jsonschema-mock-service/tasks-data/${carrier}/${dataFileName}.json`);

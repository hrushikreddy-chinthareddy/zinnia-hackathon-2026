...
export const useTransactionsHistory = ({
    ...
    useEffect(() => {
        const statusFilter = (transaction: Transaction) =>
        ...
        const getTransactions = async () => {
            try {
                setLoader(true);
                const data = await getPolicyTransactionHistory(
                    contract,
                    clientId,
                    typeDesc,
                    transactionType,
                ...
                if (data?.Items) {
                    const filteredTransactions = filter
                        ? data?.Items?.filter(statusFilter)
                        : data?.Items;
                ...
                console.error(
                    'TransactionHistory::Error retrieving transaction history',
                    e
                );
            }
        };

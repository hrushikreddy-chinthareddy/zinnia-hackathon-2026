...
const UpdateNotificationMethod = ({ recordId, policyNumber, carrier, policy }: UpdateNotificationMethodProps) => {

  const [transactionData, setTransactionData] = useState<NotificationsTransactionData>({} as NotificationsTransactionData);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchTransactions = async (recordId: string) => {
      setLoading(true);
      const transactionResponse = await getTransactionsByRecordId(recordId);
    ...
    <UpdateNotificationMethodProvider>
      {loading ? (
        ...
        <div className="flex w-full flex-col overflow-auto px-4 py-6 md:px-6 md:py-8 lg:px-8 lg:py-10">
          <UpdateNotificationMethodContainer transactionData={transactionData} policy={policy} />
        </div>
      )}
    </UpdateNotificationMethodProvider>
  </>
...
export const getServerSideProps = withPageAuthAndLogging(
  {
    getServerSideProps: async (context, loggingContext) => {
      ...
      const logCtx = { ...loggingContext, file: 'pages/claims/update-notification-method', function: 'getServerSideProps' };
      let accessToken;
      try {
        accessToken = (await getAccessToken(req, res)).accessToken;
      } catch (e) {
        logWarn('update-notification-method::Access token expired', { ...logCtx });
      ...
      try {
        ...
        if (!planCode) {
          logInfo('update-notification-method::id::Plan code not found', loggingContext);
          return {
            redirect: {
          ...
          logInfo('update-notification-method:id::Plan code found', loggingContext);
        }

        const policy = await getPolicyDetailsSsr(query.policyNumber as string, planCode, accessToken, loggingContext, true);
        ...
        logError('update-notification-method::getServerSideProps', { ...parseErrorInformation(error), ...logCtx });
        return {
          props: {},
        };
      }
  ...
  { file: 'claims/update-notification-method/index', function: 'getServerSideProps', page: 'update-notification-method' }
);

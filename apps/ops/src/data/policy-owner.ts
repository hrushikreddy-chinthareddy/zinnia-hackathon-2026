...
export const toPolicyOwnerDto = ({
    ...
    return {
        ...
        email: emails?.find(email => email.emailType === 'PERSONAL' && !isEndDated(email.endDate)),
        emailType: emails?.find(email => email.emailType && !isEndDated(email.endDate))?.emailType,
        mailingAddress: addresses?.find(address => !isEndDated(address.endDate)),
    };
};

import { FeatureType, Policy, ArrangementType, FundAccountType } from '@zinnia/api-types/types/sor';
 ...
 */
export const toAnnuityViewDetailsDto = (policy: Policy): AnnuityViewDetailsDto => {
    if (!policy) return {} as AnnuityViewDetailsDto;

    const { accountValues, coverage, withdrawalValues, policyFeatures, policyDates } = policy;
    ...
    const withdrawalAutopayDetails = policy.systematicPrograms?.find(sp => sp.arrangementType === ArrangementType.WITHDRAWAL);
    const withdrawalAutopayAmount = withdrawalAutopayDetails?.amount || 0;
    const withdrawalAutopayFrequency = withdrawalAutopayDetails?.frequency || '';
    const withdrawalAutopayNextPaymentDate = withdrawalAutopayDetails?.nextProgramDate || '';

    //TODO: Is this the correct mapping?
    const minDistDetails = policy.systematicPrograms?.find(sp => sp.arrangementType === ArrangementType.REQUIREDMINIMUMDISTRIBUTION);
    ...
    const rmdAnnuallyRequired = policy.requiredMinimumDistribution?.totalRequiredMinimumDistributionAnnualAmount;
    const rmdRemainingAmount = policy.requiredMinimumDistribution?.remainingRequiredMinimumDistributionAmount;

    //TODO: Is this the correct mapping?
    const payoutDetails = policy.systematicPrograms?.find(sp => sp.arrangementType === ArrangementType.PAYOUT);
    ...
    const annuitizationDetails = policy.policyFeatures?.find(sp => sp.featureType === FeatureType.ANNUITIZATION);
    ...
    const totalFundValue = policy?.allocation?.funds?.find(fund => fund.fundAccountType === FundAccountType.FIXED)?.totalFundValue;
    ...
    return Object.assign(
        {},
        {
            ...
            //premium
            cumulativePremiumSinceIssue: policy.accountValues?.cumulativePremiumSinceIssue || 0, //TODO: Is this the correct mapping?
            ...
            withdrawalAutopayNextPaymentDate: withdrawalAutopayNextPaymentDate || '',
            ...
            ytdWithdrawalAmount: withdrawalValues?.totalYearToDateWithdrawalTaken || 0,
            //required min distr
...
export const AnnuityDetailsViewInfo = (): DataDefinition<AnnuityViewDetailsDto>[] => {
    return [
        {

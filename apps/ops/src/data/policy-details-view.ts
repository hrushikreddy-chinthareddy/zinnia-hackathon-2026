...
export const toPolicyViewDetailsDto = (
    policy: Policy
    ...
    const minCoverageAmount =
        coverage?.coverageLayers?.[0]?.minimumCoverageAmount ?? 0;
    const maxCoverageAmount =
        coverage?.coverageLayers?.[0]?.maximumCoverageAmount ?? 0;
    ...
    return Object.assign(
        {},
        {
            ...
            minRequiredAccountValue:
                accountValues?.minimumRequiredAccountValue || 0,
            contestabilityPeriodStartDate:
                policyDates?.contestabilityStartDate || '',
            contestabilityPeriodEndDate:
                policyDates?.contestabilityEndDate || '',
            ...
            totalPremiumAmount:
                accountValues?.totalYearToDatePremiumAmount || 0,
            cumulativePremiumSinceIssue:
                accountValues?.cumulativePremiumSinceIssue || 0,
            amountRemainingUntilCurrentGuidelineLimit:
                (policy?.testValues?.guidelinePremium?.guidelineSinglePremium ||
                    0) - guidelineBasis || 0,
            ...
            amountRemainingUntilCurrentSevenPayLimit:
                (testValues?.modifiedEndowmentContract?.sevenPayLimit || 0) -
                    (testValues?.modifiedEndowmentContract?.sevenPayTestBasis ||
                        0) || 0,
            sevenPayPremiumBasis:
                testValues?.modifiedEndowmentContract?.sevenPayTestBasis || 0,
...
export const toTermLifeViewDetailsDto = (
    policy: Policy
): Partial<PolicyViewDetailsDto> => {
...
export const PolicyDetailsViewInfo =
    (): DataDefinition<PolicyViewDetailsDto>[] => {
        return [
            {
                key: 'issueDate',
                label: 'Issue date',
...
export const TermLifeDetailsViewInfo = (): DataDefinition<
    Partial<PolicyViewDetailsDto>
>[] => {
    return [
        {

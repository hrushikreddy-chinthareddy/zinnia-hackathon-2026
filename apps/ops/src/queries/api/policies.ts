...
export const downloadAsIsIllustration = async (
    planCode: string,
    policyNumber: string
): Promise<any> => {
    // browserLogInfo(
    ...
    // );
    try {
        //dev.api.zinnia.io/v1/policies/ELIULV01/ELI2689023/illustrations/as-is-inforce/pdf
        const url = `${baseAppUrl}/api/v1/policies/${planCode}/${policyNumber}/illustrations/as-is-inforce/pdf`;
        const formData = {
            carrierCode: 'ELIC',
            policyStatus: 'ACTIVE',
        };
        ...
        browserLogError(
            'searchPolicies::An error occurred while getting policies',
            {
                ...
                planCode,
                error,
                url: `${baseAppUrl}/api/polic/v1/policies/${planCode}/${policyNumber}/illustrations/as-is-inforce/pdf`,
            }
        );
        return error.response;
    }

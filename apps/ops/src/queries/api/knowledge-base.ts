...
import { FollowUpChainResponse, FollowUpResponse, UserResponse } from '@xd/api-types/dist/generated-types/knowledgebase';
import axios, { AxiosResponse } from 'axios';

import { ClientDetailsResponse, DocumentsDisplayType, FeedbackType, GetAllUserDetailsResponse, SortBy, SortDirection } from '@deps/types/knowledge-base';
...
export const getFollowupMessages = async (
  ...
  if (!messageId) {
    browserLogError(
      'Error getting follow up messages:: missing messageId'
    );
    return null;
  }
...
export const sendFollowupMessage = async (
  ...
  try {
    const { data } = await client.post(url, {
      ...
      followUpQuestion,
      clientId,
      parentFollowUpId: parentFollowUpId === null ? undefined : parentFollowUpId
    });
    return data ?? null;
  } catch (error) {

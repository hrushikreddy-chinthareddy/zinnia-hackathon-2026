...
import { ChatbotMessage, MessageRole, UserMessage } from "@deps/types/knowledge-base";
...
const KnowledgeBaseContext = createContext<KnowledgeBaseContextState>(KnowledgeBaseContextDefaultValues);
...
const KnowledgeBaseProvider = ({ children, opsUserData, sessionId: sessionIdProp = '' }: KnowledgeBaseContextProps) => {
  const defaultClientId = opsUserData?.client?.find((client) => client.default === true)?.id ?? '';
  const [selectedClientId, setSelectedClientId] = useState<string>(defaultClientId);
  const [sessionId, setSessionId] = useState<string>('');
  const [currentMessages, setCurrentMessages] = useState<(UserMessage | ChatbotMessage)[]>([]);
  ...
  const viewChatHistory = async (sessionId: string) => {
    ...
    try {
      ...
      if (historyMessagesResponse) {
        const historyMessages = historyMessagesResponse?.messages.content;

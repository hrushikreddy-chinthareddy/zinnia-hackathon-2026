...
Types are imported using the `@zinnia/api-types/types` alias to maintain compatibility with the monorepo structure:
...
This setup was migrated from `packages/api-types` to prepare for Consumer Experience becoming a standalone repository. The `@zinnia/api-types/types` import alias is maintained for easy migration - when the repo is split, existing imports will continue to work without changes.
